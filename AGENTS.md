...
>
> **Current focus:** SP-A reader-route spec approved and implementation plan committed. Plan: [docs/superpowers/plans/2026-05-28-reader-route-privacy-discoverable.md](docs/superpowers/plans/2026-05-28-reader-route-privacy-discoverable.md) (15 tasks, TDD-shaped, bite-sized commits). Spec: [docs/superpowers/specs/2026-05-28-reader-route-privacy-discoverable-design.md](docs/superpowers/specs/2026-05-28-reader-route-privacy-discoverable-design.md). Scope summary: new `/[locale]/books/[bookId]` reader route + privacy enum extension (adds FRIENDS) + reuse existing-but-unused `books.explorable` column renamed to `discoverable` (saves a column add) + `canReadBook()` access helper + Sharing UI (wizard step + Details page section) + editor entry points (toolbar Preview button + binder-header title link). FRIENDS visibility is settable but resolves to author-only until SP-B Friendships ships. Migration preserves current /discover visibility: existing PUBLIC+PUBLISHED books get `discoverable=true`. Old `/discover/book/[bookId]` URLs 308-redirect to the new route; shared reader components (`ChapterList`, `CommentsPanel`, `SocialActions`) hoist from `discover/_components/` to `(public)/_components/`. Awaiting Chris's choice of execution mode (subagent-driven vs inline).
> **Active branch:** `main`
> **Last commit:** docs(reader): SP-A implementation plan — 15 tasks, TDD, frequent commits
>
...
>
> **Next concrete step when resuming:** Ask Chris which execution mode for the SP-A plan: (1) subagent-driven (recommended for a 15-task plan — fresh subagent per task, review between, keeps main context clean); (2) inline via executing-plans (batch with checkpoints). Once chosen, kick off Task 1 (schema migration: extend `book_visibility` enum with FRIENDS, rename `explorable`→`discoverable`, add composite index, backfill existing PUBLIC+PUBLISHED books to `discoverable=true`). The spec and plan are locked — do not re-litigate without explicit instruction. After SP-A ships, the next decisions in order are: (a) SP-B Friendships subsystem (separate brainstorm — symmetric request/accept like the old beehive-books-online had); (b) Hive ↔ Binder integration via the locked spec at [docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md](docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md); (c) Phase 9 polish (referral codes, growth analytics, plan-upgrade nudges); (d) Stripe dashboard webhook configuration for live monetization; (e) a `/settings` index page.
