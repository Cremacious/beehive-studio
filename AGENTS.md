...
>
> **Current focus:** **Linked Series execution — Tasks 1-4 shipped.** Plan: [docs/superpowers/plans/2026-05-28-linked-series.md](docs/superpowers/plans/2026-05-28-linked-series.md). T1 (`9849971`): `normalizeSeriesKey` helper + 7 tests. T2 (`d99bc70`): `getSeriesNeighbors` server helper + 6 tests. T3 (`2142f3a`): BookSummary projection extension. **T4 (`d5bcec1`)**: shared `SeriesLine` component at `components/book/series-line.tsx` renders "Book N · *Series*" or "Part of *Series*" (mono 11px, 0.05em). Added to 3 surfaces: library card, discover card, profile tile at `(public)/u/[username]` (path correction — profile lives in public group per Phase 7). `DiscoverBook` + `ProfileBook` types + their SELECTs both extended with seriesName/seriesNumber. Each surface guards `book.seriesName &&` before rendering as belt-and-suspenders with the component's internal null-check. Spec ✅, quality ✅. **Next:** T5 = library "By series" sort + clustering with subheaders ("Series Name (N books)" / "Standalone (N)"); T6 = book reader hero series line + `<SeriesFooter>` prev/next; T7 = wizard + Details explainer copy; T8 = AGENTS.md sync + final verification. 175/175 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(series): show 'Book N of <Series>' on library / discover / profile cards
>
