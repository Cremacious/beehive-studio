...
>
> **Current focus:** **SP-A — Reader Route + Privacy/Discoverable COMPLETE** (15/15 tasks shipped + 2 follow-ups). End-to-end: studio editor → toolbar Preview (Eye icon) OR binder-header title click → `/[locale]/books/[bookId]` reader (canReadBook-gated) → chapter reader at `/read/[chapterId]` with mark-as-read. Library kebab `Preview` shipped as `67c3a39`. **Just shipped (`3236e18`)**: T12 visual fix — the toolbar Preview button was using a raw `<NextLink>` with `tbtnClass()`, but anchor color rules disagreed with `text-foreground/65` and the Eye rendered nearly invisible. Swapped to `<ToolbarButton onClick={() => router.push(...)}>` — same wrapper every other VIEW-zone button uses — eliminating the divergence; Eye now matches the other icons. Privacy fully threaded: wizard Sharing step + Details Sharing section; PRIVATE/FRIENDS/PUBLIC + discoverable toggle. FRIENDS settable but resolves to author-only until SP-B ships. Old `/discover/book/*` 308s to `/books/*`. T13 still has 2 known MINOR quality concerns ("ship as-is"): dual-click race on binder title + h2→a semantic regression. **Pattern note for future Link-styled icon buttons in this toolbar:** prefer `ToolbarButton` + `router.push` over raw `<Link>` to inherit consistent `text-foreground/65` rendering. 137/137 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** fix(editor): Preview button uses ToolbarButton + router.push, not NextLink
>
