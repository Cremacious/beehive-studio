...
>
> **Current focus:** **Drag-into-folder execution via subagent-driven mode — Task 1 of 5 shipped.** Plan: [docs/superpowers/plans/2026-05-28-drag-into-folder.md](docs/superpowers/plans/2026-05-28-drag-into-folder.md). **T1 (`32c9626`)**: pure helpers + tests at `lib/binder/drop-rules.ts` + `lib/binder/__tests__/drop-rules.test.ts`. Exports: `BinderItemType` (8-type union mirroring BinderItemRow), `BinderItemLite`, `getAcceptedChildTypes`, `canNest` (rejects self/type-mismatch/cycle; cycle walk via parentId Map with `seen` defense against corrupt data), `classifyDropZone` (folders 6px edge / midpoint else / null when out-of-rect). 17 unit tests pass (plan miscounted as 18 — one `it` block packs the 6 non-container types). Spec ✅ + quality ✅. **Next:** T2 = wire drop-intent state + onDragOver + auto-expand timer + intent-driven handleDragEnd in `binder-tree.tsx`; T3 = render before/middle/after indicators in `binder-item.tsx` via extended `BinderTreeContext`; T4 = AGENTS.md sync; T5 = final verification. 154/154 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(binder): drop-rules helpers + unit tests
>
