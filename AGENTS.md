...
>
> **Current focus:** **Chapter-Status Publish-Gate — Tasks 1-4 shipped.** Plan: [docs/superpowers/plans/2026-05-28-chapter-status-publish-gate.md](docs/superpowers/plans/2026-05-28-chapter-status-publish-gate.md). T1 (`34ff8da`): helper + tests. T2+3 (`b73959b`): book reader page projects status+updatedAt; ChapterList gates per-row with locked badge + freshness label; hero CTA + lastReadChapter both filtered through readerVisibleChapters. **T4 (`7c65b11`)**: chapter reader page locked-placeholder for direct-URL access. Added `chapters.status` + `books.userId` to inline projections; `isChapterReaderVisible` import; gate placed AFTER `canReadBook` + chapter fetch, BEFORE `markChapterReadAction` so locked direct-URL hits don't mark as read. `LockedChapterPlaceholder` with Clock icon + back-link to `/books/[bookId]`. Spec ✅, quality ✅. **Next:** T5 = binder color dots for chapter rows (may extend `BinderItemRow` projection to expose status); T6 = metadata panel explainer + per-pill subtitles; T7 = AGENTS.md sync; T8 = final verification. 162/162 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(reader): locked placeholder for non-visible chapters by direct URL
>
