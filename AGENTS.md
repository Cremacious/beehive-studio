...
>
> **Current focus:** **Hives redesign — H1 EXECUTION IN PROGRESS (2 of 17 tasks shipped).** Subagent-driven execution flow: dispatch implementer → spec reviewer → code-quality reviewer → checkpoint with Chris before next task. Plan at `docs/superpowers/plans/2026-05-29-h1-hive-foundation.md`. **Task 1 (schema migration) COMPLETE** in 3 commits: (1) `05bdf3b` — `db/schema/hive.ts` + `scripts/migrate-h1.ts`: enum collapse 5→4 (EDITOR→MODERATOR, PROOFREADER→CONTRIBUTOR), `hives.discoverable boolean`, partial UNIQUE `hives_book_id_unique WHERE book_id IS NOT NULL` (H2 will tighten — intentional), `hives.book_id` FK→CASCADE, new `hive_activity` table + 10-value `hive_activity_type` enum. Plus minimal spillover patches in `_helpers.ts` + `hive.actions.ts` for EDITOR/PROOFREADER → MODERATOR string rewrites to keep tsc clean. (2) `570082f` — fix orphaned FK from migration; live DB now has exactly ONE `hives.book_id` FK (CASCADE); script idempotent via `DROP CONSTRAINT IF EXISTS` for both possible names. (3) `8b16a8b` — wrap `CREATE TYPE hive_member_role_new` in `DO $$ EXCEPTION WHEN duplicate_object` for swap-dance partial-failure re-runnability + align H1 spec to 10-value activity enum (originally listed 7, plan/code use 10 for H3 forward-compat). Live DB verified: discoverable column present, partial UNIQUE intact, CASCADE FK, role enum has 4 values, hive_activity table created, tsc clean. **Task 2 (permission helpers) COMPLETE** in commit `a759f10` — `lib/hive/permissions.ts` exports `HiveRole` type + 3 require-* helpers (`requireHiveMember`/`requireHiveMod`/`requireHiveOwner`, throw on auth failure, return role on success) + 8 pure predicates (`canEditWiki`/`canSubmitChapter`/`canReviewSubmissions`/`canAnnotate`/`canSuggestEdits`/`canEditOutline`/`canManageMembers`/`canDeleteHive`); 8/8 vitest cases green, tsc clean. Remaining tasks 3–17 deferred to next session: T3 getBookHive helper, T4 recordHiveActivity + getHiveActivityFeedAction, T5–T10 server actions (createHive 3 paths, updateHive coercion, getUserHivesView, member_joined wiring, discover rename, delete getCommunityFeedAction), T11–T14 UI (/studio Hives section, CreateHiveModal, BinderHiveFooter, /community rewrite), T15 /discover swap confirm, T16 /hive Settings+Members+stubs, T17 ship commit. **Next concrete step:** resume by dispatching Task 3 (`lib/hive/get-book-hive.ts` — single React-cache-memoized reverse lookup `getBookHive(bookId) → { hiveId } | null`, plus 2 mocked-DB vitest cases). Task 3 is also pure helper, fully mechanical from the plan code; same dispatch shape as Task 2. Full Hives redesign spec stack (H1–H5) committed earlier: `7fc7cb7` H1, `2148ab6` H2, `8282b0d` H3, `b742908` milestone-removal amendment, `f1767a9` H4, `d11db57` H5. Linked Series + Chapter Status + Drag-into-Folder + Character Redesign + Delete Book all still awaiting Chris's manual smoke (carried from prior session).
> **Active branch:** `main`
> **Last commit:** feat(hive): permission helpers (4 roles × 8 predicates) + require* functions
>
