...
>
> **Current focus:** **Chapter-Status Publish-Gate execution — Tasks 1, 2+3 shipped.** Plan: [docs/superpowers/plans/2026-05-28-chapter-status-publish-gate.md](docs/superpowers/plans/2026-05-28-chapter-status-publish-gate.md). T1 (`34ff8da`): `isChapterReaderVisible` helper + 5 unit tests. **T2+3 (`b73959b`, combined)**: reader page (`books/[bookId]/page.tsx`) chapter projection now includes `status`+`updatedAt`; `isAuthor` computed once and threaded to ChapterList; hero "Start/Continue Reading" CTA + `lastReadChapter` both gated through `readerVisibleChapters` for non-authors (no broken links to locked chapters); ChapterList branches per-row — locked = non-clickable `<div>` with "Draft — coming soon" badge + `aria-disabled`, visible = `<Link>` with "Updated MMM DD, YYYY" label. `formatUpdatedLabel(Date | string)` handles Next.js serialization. Spec ✅, quality ✅. **Next:** T4 = chapter reader page locked placeholder (direct-URL access guard); T5 = binder color dots for chapter rows; T6 = metadata panel explainer paragraph + per-pill subtitles; T7 = AGENTS.md sync; T8 = final verification. 162/162 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(reader): chapter list gates by status; show Updated label on visible chapters
>
