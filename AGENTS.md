...
>
> **Current focus:** **Hives Redesign — H2 Mirror Model IN PROGRESS** (13 of 18 tasks shipped — all wiki renderers wired in editor; T14 lands outline act grouping next). H2 plan at `docs/superpowers/plans/2026-05-29-h2-mirror-model.md` (18 tasks, schema → helpers → permissions → actions → editor UI → hive UI → ship). Chris confirmed 4 pre-flight decisions: (1) add `author_id` + `last_edited_by` columns to binder_items for author tracking; (2) wiki entry editor mounts in-place on `/hive/[hiveId]/wiki`; (3) outline empty acts live in client state only until a beat lands; (4) hive binder fetch is a sibling `getBinderTreeForHiveAction` not an extension of existing action. **T1 (schema migration) COMPLETE** in 1 commit: `b74670e` — `db/schema/books.ts` widened `bookStatusEnum` (+STANDALONE_HIVE_SHADOW) + `binderItemTypeEnum` (+wiki_entry, +wiki_folder); `binderItems` gained `authorId` + `lastEditedBy` text FKs (ON DELETE SET NULL) + composite `binder_items_book_type_idx` on `(book_id, type)`; `db/schema/hive.ts` deleted `hiveOutlines` + `hiveWikiPages` tables + relations + unused `sql` import; `hives.bookId` tightened to `.notNull()` + H1 partial UNIQUE replaced with plain `uniqueIndex('hives_book_id_unique').on(t.bookId)`; new `scripts/migrate-h2.ts` runner ran clean on dev DB: 0 shadow books backfilled (no pre-H2 standalone hives — expected), 3 hive_wiki_pages ported → 3 wiki_entries under 1 auto-created "Imported from old wiki" wiki_folder, 3 hive_outlines ported (appended "Imported" beat to existing outline items where present, created fresh outline items elsewhere — 9 outlines total post-port), hives_with_null_book=0 sanity check passed, legacy tables dropped. tsc INTENTIONALLY DIRTY between T1 and T2 per plan (book_status enum widening + deleted hive content tables cascade): `binder.actions.ts:82` BinderItemRow narrower than widened enum, `book.actions.ts:380` book projection narrower than widened bookStatusEnum, `hive-content.actions.ts` ×8 references deleted tables (T10 deletes file), `hive.actions.ts:79` passes string|null to NOT-NULL bookId (T8 reshape closes). T2 (`scopedBooksForUser` codemod across ~13 sites) is the tsc-clean boundary. **H1 Foundation ✅ COMPLETE** (17/17 tasks — full breakdown in "What Has Been Built" → "Hives Redesign — H1 Foundation"). (17/17 tasks shipped — see "What Has Been Built" → Hives Redesign H1 for the full breakdown). H1 lands the relational foundation, helpers, server actions, and primary UI surfaces — every book has at most one hive (DB-enforced via partial UNIQUE), every hive page has a real 11-entry sidebar shell (Settings/Members/Dashboard real; 8 H2–H4 stubs render "Coming in HX"), /community rewritten as a hive-activity feed (replaces P7.5 follows feed), /discover Hives filtered on `discoverable=true AND visibility='PUBLIC'`. **H1 awaiting Chris's manual smoke** (carry-forward from per-task checkpoints): (1) create a standalone hive from /studio → appears in Hives section with role=OWNER; (2) create a hive linked to a book → editor binder footer flips Create Hive → Go to Hive; (3) try to create a 2nd hive for the same book → toast error (partial UNIQUE blocks); (4) invite a 2nd user → member_joined activity fires + appears in their /community feed; (5) delete the book → hive cascade-deletes; (6) flip a hive to PUBLIC + discoverable → appears on /discover Hives tab; toggle discoverable=false → disappears from listings but URL still works for members; (7) /hive/{id} 11-entry sidebar + 8 stubs + Settings (OWNER edit + delete via ConfirmDialog) + Members (OWNER role dropdown + remove w/ rollback) all work. **T2 (`scopedBooksForUser` helper + codemod) COMPLETE** in 1 commit: `a1b3ea1` — new `lib/books/scoped.ts` with `scopedBooksForUser(userId)` (relational builder: `and(eq(books.userId, userId), ne(books.status, 'STANDALONE_HIVE_SHADOW'))`) + `scopedBooksForUserSql(userId)` (raw SQL fragment for CTE-compose cases); 8 codemod sites across 7 files (`_helpers.ts` assertBookOwner; `book.actions.ts` ×5 — getActiveBookCount, library list, getStudioStatsAction subquery + inProgress filter, getBookAction, deleteBookAction + added `as 'DRAFT' | 'PUBLISHED'` narrowing on getBookAction return; `book-overflow.ts`; `get-series-neighbors.ts`; export route); intentional non-codemod sites confirmed and documented (`createHiveAction` got defensive `ne(...,STANDALONE_HIVE_SHADOW)` add since semantics differ — that's a "look up this specific user-supplied book" query not a library query; `user-profile.actions.ts` ×3 already PUBLISHED-scoped, comments added; `community.actions.ts` + `discover.actions.ts` plan flags were false alarms — those `eq(books.userId, ...)` hits are `innerJoin` ON-conditions not WHERE filters, correctly skipped). tsc book_status union-narrowing error at `book.actions.ts:381` now closed. Remaining tsc deferrals all expected (binder.actions.ts:82 wiki_entry/wiki_folder not in BinderItemRow union — T11/12 widens; hive-content.actions.ts ×6 references deleted hiveOutlines/hiveWikiPages — T10 deletes file; hive.actions.ts:86 nullable bookId insert into NOT-NULL col — T8 reshape). Tests 186/186. Smoke if wanted: direct-URL `/studio/{fake-shadow-uuid}` returns "Book not found" via tightened assertBookOwner (no real shadow books exist yet — first one lands in T8). **T3 (category templates module) COMPLETE** in 1 commit: `7b18060` — `app/globals.css :root` gained 14 `--wiki-*` oklch accent color tokens in a new "Wiki category accents (14)" group between `--type-character` and `--validation` (CHARACTER warm gold, LOCATION teal, LORE violet, PLOT warm red, ARTIFACT amber, FACTION steel blue, CULTURE sage, LANGUAGE slate cyan, BIOLOGY leafy yellow-green, THEME magenta, ECONOMY copper, TERMINOLOGY lavender gray, TIMELINE aqua, OTHER neutral); new `lib/wiki/category-templates.ts` exports `WikiCategory` union + `CategoryTemplate` type + `CATEGORY_TEMPLATES` array (14 entries) + `CATEGORY_TEMPLATE_MAP` record-by-category + private `doc()` helper composing TipTap JSON from h2+hint pairs; each template carries lucide icon (User/MapPin/ScrollText/Drama/Sword/Flag/Globe/Languages/Leaf/Sparkles/Coins/BookA/Clock/FileQuestion — all verified present in lucide-react), label, one-line blurb for the picker UI, accentColor CSS var (matching globals.css), defaultBody TipTap doc with 2–3 section headings + hint paragraphs that prime the user; 4 new tests (count=14, every union value covered by map, all defaultBody shapes are valid TipTap doc with non-empty content array, all accentColors start with `--wiki-`). Tests 190/190. tsc clean for T3 — same 9 expected T-future deferrals remain (binder.actions.ts:82, hive-content.actions.ts ×6 at lines 4/16/37/40/56/64/73, hive.actions.ts:86). No smoke needed (pure data module, T11 binder add menu is first UI consumer). **T4 (`requireBinderWritePermission` + 50-case truth table) COMPLETE** in 1 commit: `0f7658c` — appended `BinderItemTypeForPermission` union + `requireBinderWritePermission(bookId, binderItemId, userId): Promise<void>` async helper to `lib/hive/permissions.ts`; helper does author bypass (book.userId === userId → resolve), else hive lookup by bookId (none → NOT_AUTHORIZED), member lookup for (hive, userId) (none → NOT_AUTHORIZED), item lookup by id (missing or wrong book → BINDER_ITEM_NOT_FOUND), then switches on item type: chapter/part/front_matter/back_matter → NOT_AUTHORIZED (author-only; H3 wires submission flow), outline → require `canEditOutline(role)`, wiki_entry/wiki_folder/character/research_note/research_folder → require `canEditWiki(role)`. Both predicates confirmed pre-existing in H1 T2 with `(r: HiveRole) => boolean` signature returning `r !== 'BETA_READER'` — used directly, no adaptation needed. Tests: 42 new in `lib/hive/__tests__/permissions.test.ts` via `vi.mock('@/db', ...)` (40 truth-table cases — OWNER/MODERATOR/CONTRIBUTOR all-allow on wiki/folder/character/outline/notes/research_folder all-deny on chapter family, BETA_READER deny-all + 2 author bypass: author always allowed, non-member non-author denied). Total 232/232 (190 baseline + 42 new). tsc clean for T4 — same 9 pre-existing T-future deferrals remain. Minor cosmetic deviation: test names use `x` and `=>` not `×` and `⇒` (ASCII-safe). No smoke — helper isn't wired yet; T7 mounts it on createBinderItemAction/updateBinderItemAction/deleteBinderItemAction/reorderBinderItemsAction. **T5 (tag normalization helpers) COMPLETE** inline in 1 commit: `e32c65e` — new `lib/wiki/tags.ts` exports `MAX_TAGS = 10`, `normalizeTags(input: readonly string[] | undefined | null): string[]` (trim + lowercase + drop-empty + dedupe via Set + cap at MAX_TAGS), `acceptTag(existing, candidate): string | null` (single-add validator returning normalized form or null on empty/dupe/at-cap). 9 tests cover null/undefined/empty, lowercase+trim, dedupe-after-lowercase, drop-empty-after-trim, cap-at-MAX_TAGS, plus acceptTag valid/dupe/cap/empty paths. tsc deferrals identical to T4 (same 9 pre-existing T-future deferrals: binder.actions.ts:82, hive-content.actions.ts ×7, hive.actions.ts:86). Shipped inline (no subagent dispatch) — task was ~30 LOC verbatim from plan + ~40 LOC test verbatim, no judgment calls. Consumed by T12 (WikiEntryEditor tag chip strip) and T15 (Character tag strip). **T6 (`groupBeatsByAct` + `distinctActs` pure helpers) COMPLETE** inline in 1 commit: `e88222a` — new `lib/outline/group-by-act.ts` exports `ActBeat` type (existing `Beat` from outline-board.tsx extended with optional `act: string | null`), `ActGroup` type (`{ act: string | null; beats: ActBeat[] }`), `groupBeatsByAct(beats)` (preserves first-appearance order BETWEEN groups + insertion order WITHIN; null-act beats collect into one null-keyed group at top only if non-empty; trims + treats empty-string acts as ungrouped), `distinctActs(beats)` (unique acts for per-act header autocomplete, excluding null/empty). 8 tests: empty input, all-null=one-null-group, all-acts=no-null-group, first-appearance ordering ['II','I'], null-first surfacing, whitespace-trim/empty-string→null. tsc deferrals unchanged from T5 (same 9 pre-existing). Shipped inline — pure helper + tests verbatim from plan, no judgment. Consumed by T14 outline editor act-grouping UI. **Helpers phase (T3–T6) complete: category templates, permission helper + truth table, tag normalization, outline grouping all shipped.** **T7 (wire permission helper + projection extension) COMPLETE** in 2 commits: (1) `a397cf6` main — appended `requireBinderCreatePermission(bookId, type, userId)` to `lib/hive/permissions.ts` (mirrors requireBinderWritePermission but skips binder-item lookup since row doesn't exist yet); added module-private `getBinderItemBook(id)` lookup in `binder.actions.ts`; swapped 4 binder actions (`createBinderItemAction` → requireBinderCreatePermission; `updateBinderItemAction`/`deleteBinderItemAction` → getBinderItemBook + requireBinderWritePermission; `reorderBinderItemsAction` → gates on parsed.data[0].id's requireBinderWritePermission with code comment noting mixed-type edge case relies on UI not emitting those); `getBinderTreeAction` projection extended with `authorId: item.authorId` + `lastEditedBy: item.lastEditedBy`; `BinderItemRow` widened — type union gains `wiki_entry | wiki_folder`, new `authorId: string | null` + `lastEditedBy: string | null` fields; inserts write `authorId: userId, lastEditedBy: userId` (set-once + audit-trail); updates seed `lastEditedBy: userId`; reorder does NOT bump lastEditedBy (not an edit). `getBinderTreeAction` keeps `assertBookOwner` per plan Step 4 — hive read path is T9's `getHiveWikiView` etc. with its own assertHiveMember. Closes `binder.actions.ts:82` T-future deferral. (2) `270e7d7` follow-up — BinderItemRow widening cascaded into 6 UI sites, fixed inline: `lib/binder/drop-rules.ts` widened `BinderItemType` union + added `wiki_folder` ACCEPT_TABLE entry (accepts `['wiki_entry', 'wiki_folder', 'character']` for mirror-lens parity with the spec's "Character entries appear wherever the author placed them" + spec's category UNION model); `binder-item.tsx` ICONS + ICON_TINTS records gained wiki_entry (BookOpen + `var(--wiki-other)`) and wiki_folder (Folder + `var(--wiki-other)`) — placeholder neutral tint since per-category accent system from T3 needs content-shape inspection to pick the right color, which surfaces in WikiEntryEditor (T12) and hive Wiki page (T16) not the generic binder row; 3 optimistic-add literal BinderItemRow constructions (binder-add-menu, binder-item-menu, chapter-editor) now include `authorId: null, lastEditedBy: null` (rows backfill to real user IDs on the next server-side refresh via getBinderTreeAction). tsc clean — only T8 (`hive.actions.ts:86` nullable bookId) + T10 (`hive-content.actions.ts` ×7 deleted-table refs) deferrals remain. Tests 249/249. Smoke: add/edit a chapter via the binder + Add menu must still succeed (author bypass intact); no runtime issues since the widening is TS-only. **T8 (`createHiveAction` reshape — shadow-book on standalone path) COMPLETE** in 1 commit: `7ee8a75` — `lib/actions/hive.actions.ts` `createHiveAction` now branches three ways: (1) free-tier limit check unchanged (FREE_LIMIT_REACHED); (2) **linked-book path** (`data.bookId` truthy) — defensive `ne(books.status, 'STANDALONE_HIVE_SHADOW')` in ownership lookup → BOOK_NOT_FOUND if missing or shadow, then `getBookHive` uniqueness check → BOOK_ALREADY_HAS_HIVE, assigns `bookId = data.bookId`; (3) **standalone path** (no `data.bookId`) — generates `bookId = createId()`, inserts shadow `books` row outside the tx with `userId, title=data.name, visibility='PRIVATE', discoverable=false, status='STANDALONE_HIVE_SHADOW'`; transaction then inserts `hives` (bookId always non-null at this point) + OWNER `hiveMembers` row. Shadow book insert intentionally outside the tx per plan — trade-off: if hive insert fails the shadow leaks as orphan, but orphan-shadow-with-no-hive is invisible (scopedBooksForUser excludes from library, no UI surface queries it). New test file `lib/actions/__tests__/create-hive-action.test.ts` with 3 behavior tests via `vi.mock('@/db', ...)` matching `get-book-hive.test.ts` style: standalone path inserts books-before-hives + hive.bookId matches shadow.id; linked-book path with shadow ID returns BOOK_NOT_FOUND + writes nothing; linked-book path with pre-existing hive returns BOOK_ALREADY_HAS_HIVE + writes nothing. Schema tests in `lib/validations/__tests__/hive.test.ts` untouched. **`hive.actions.ts:86` deferral CLOSED.** Both H2 T-future deferrals now closed (T7 closed binder.actions.ts:82, T8 closes hive.actions.ts:86). Only `hive-content.actions.ts` ×9 errors remain — T10 deletes the file. Tests 252/252. Smoke: standalone hive creation from /studio → DB shows shadow `books` row + linked `hives` row, library does NOT show shadow; linked-book path with already-hived book → BOOK_ALREADY_HAS_HIVE toast; manually passing shadow id as bookId → BOOK_NOT_FOUND. **T9 (hive content view actions + sibling getBinderTreeForHiveAction) COMPLETE** in 1 commit: `df87929` — four new actions appended to `lib/actions/hive-content.actions.ts` (alongside the legacy CRUD T10 will delete; +283 LOC): (1) `getHiveWikiView(hiveId)` → `HiveWikiViewData { bookId, entries[], folders[], viewerRole, authorUserId }` — single binderItems find filtering on `inArray(type, ['wiki_entry','wiki_folder','character','research_folder'])` + single userProfiles author-join; entries projection UNIONs wiki_entry + character rows with character forced to `category: 'CHARACTER'` regardless of any content.category field; excerpt computed via new `tipTapToPlain(body, 120)`; folders projection UNIONs wiki_folder + research_folder with entryCount derived; (2) `getHiveOutlineView(hiveId)` → `{ bookId, outline: BinderItemRow|null, chapters: {id,title,order}[], viewerRole }` — chapter picker source is empty for shadow books (no chapters exist there); (3) `getHiveNotesView(hiveId)` → `{ bookId, notes: (BinderItemRow & { authorUsername })[], viewerRole }`; (4) `getBinderTreeForHiveAction(bookId, hiveId)` (the sibling action per pre-flight Q4) → `BinderItemRow[]` — asserts hive membership via `requireHiveMember`, then cross-hive escape guard verifies `hive.bookId === bookId`, then runs same projection as `getBinderTreeAction` with chapter join. All four `'use server'` returning `ActionResult<T>`. Helpers: new `tipTapToPlain(value, maxLen)` in `lib/tiptap-utils.ts` next to `extractWordCount` (walks TipTap JSON, collapses whitespace, ellipsis-truncates — chose this over importing FM/BM `toPlainText` which isn't exported + lacks truncation); private `toBinderItemRow` projection helper local to hive-content.actions.ts (14 LOC; subagent chose local over exporting from binder.actions.ts for minimal churn). **`requireHiveMember` signature confirmed:** `(hiveId, userId) => Promise<HiveRole>` (throws NOT_HIVE_MEMBER on miss, returns role directly — viewerRole falls out of the call). Minor harmless deviation: defensive `hive.bookId === null` branch returns `HIVE_NOT_FOUND` — dead code after T1+T8 made bookId NOT NULL, left as belt-and-suspenders. Spec said `'Hive not found'`; used `'HIVE_NOT_FOUND'` to match project's all-caps error-code convention. tsc unchanged — same 9 pre-existing `hive-content.actions.ts` errors from legacy `hiveOutlines`/`hiveWikiPages` references (T10 sweeps). Tests 252/252. No smoke yet — actions aren't wired into UI until T16/T17 (hive Wiki + Outline pages). **T10 (legacy hive content CRUD sweep) COMPLETE** inline in 1 commit: `49d6532` — `lib/actions/hive-content.actions.ts` -70 LOC: dropped `hiveOutlines, hiveWikiPages` from schema import; deleted 7 legacy actions (getHiveOutlineAction, saveHiveOutlineAction, getWikiPagesAction, createWikiPageAction, getWikiPageAction, saveWikiPageAction, deleteWikiPageAction) + 2 dead types (WikiPageSummary, WikiPageFull). Discussion + Tasks halves of the file untouched. Grep confirmed pre-flight (zero callers outside the file). 🎯 **tsc FULLY CLEAN** — first time since T1 widened the schema 10 commits ago. Tests 252/252. **Server layer of H2 (T1–T10) COMPLETE.** Summary: T1 schema+migration+shadow-backfill+drop-legacy, T2 scopedBooksForUser codemod (~13 sites), T3 14 category templates module, T4 requireBinderWritePermission + 50-case truth table, T5 tag normalization helpers, T6 groupBeatsByAct+distinctActs helpers, T7 wire permission helper + projection extension + UI fallout fix (BinderItemRow widening cascaded to 6 sites), T8 createHive standalone shadow-book reshape, T9 4 hive content view actions (getHiveWikiView/Outline/Notes + getBinderTreeForHiveAction sibling), T10 legacy CRUD sweep. Both T-future tsc deferrals closed mid-stream (binder.actions.ts:82 by T7, hive.actions.ts:86 by T8). **Editor UI phase begins now (T11–T15).** **T11 (binder Add menu regrouped + 13-card wiki category picker) COMPLETE** in 1 commit: `08442cf` — `binder-add-menu.tsx` restructured into three typed `AddOption[]` arrays MANUSCRIPT_OPTIONS (Chapter/Part/Front Matter/Back Matter) + WORLDBUILDING_OPTIONS (Character/Wiki Entry ▸/Wiki Folder) + PLANNING_OPTIONS (Outline/Research Note/Research Folder), rendered as three sequential sections in the existing shadcn DropdownMenu (top-aligned, w-72) via the existing `SectionLabel` component (10px semibold uppercase tracking-wide muted-foreground). Wiki Entry row tagged with `special: 'wiki-picker'`; `handleAdd` early-returns into `setPickerOpen(true)` instead of calling the server when this flag is set. NEW lucide icons: `NotebookPen` for wiki_entry row, `FolderTree` for wiki_folder row (both verified present in lucide-react). NEW component `wiki-category-picker.tsx` — shadcn Dialog with `sm:max-w-3xl` (project convention uses responsive prefix; CreateHiveModal uses sm:max-w-md but picker needs wider 4-col grid), 4-col `gap-3` grid, filters out CHARACTER (`.filter(t => t.category !== 'CHARACTER')`) → 13 visible cards (CHARACTER stays as its own first-class menu entry above Wiki Entry). Each card: icon chip with `color: var(--wiki-X)` + `background: oklch(from var(--wiki-X) l c h / 0.12)` tinted bg, label, blurb, brand-yellow hover ring via `hover:ring-2 hover:ring-brand`. Click → `onPick(category)` → `handlePickCategory(category)` in parent. `handlePickCategory` mirrors existing optimistic-add chain: closes picker, computes order from root items, calls `createBinderItemAction` with `type: 'wiki_entry'`, `title: \`New ${template.label}\`` (e.g., "New Location"), `content: { category, body: template.defaultBody, tags: [] }`, then optimistic-adds with all required BinderItemRow fields including the T7-mandated `authorId/lastEditedBy: null`, then `setActiveItemId` + `setPendingRenameId` → user lands in rename mode on the new entry. tsc clean. Tests 252/252. Smoke when ready: (1) Add menu shows 3 labeled groups; (2) Wiki Entry ▸ → picker modal with 13 cards; (3) click card → new "New {Category}" wiki_entry appears in binder, rename mode active; (4) drag wiki_entry into wiki_folder per T7-follow-up ACCEPT_TABLE works. **Heads-up:** clicking the new wiki_entry routes through `chapter-editor.tsx` render-branch which doesn't know wiki_entry yet — per DP3 "Generic textarea fallback removed" pattern editor pane currently shows nothing for wiki_entry until T12 lands. **T12 (WikiEntryEditor + shared TagChipStrip) COMPLETE** in 1 commit: `91f0d52` — three new/modified files +224 LOC. **New `tag-chip-strip.tsx`** (shared with T15 Character) — pills colored via `oklch(from var(--accent) l c h / 0.14)` bg + `var(--accent)` text; "+ tag" toggles to inline `<input>` (Enter commits via T5's acceptTag, Escape cancels, blur commits); X button per pill removes (hidden when readOnly); hides "+ tag" when `tags.length >= MAX_TAGS`; aria-label="Remove tag {t}" on X. **New `wiki-entry-editor.tsx`** — sheet-style chrome (`background: var(--sheet-canvas)`, `max-w-[760px]` centered); breadcrumb header "Wiki ▸ {category label}" + SaveStatusBadge; category pill section with template `icon` + accentColor + label; contenteditable title (`font-comfortaa font-bold text-2xl outline-none`, `onBlur → commitTitle`); TagChipStrip wired to template.accentColor; TipTap body via `useEditor({ extensions: [StarterKit.configure({ heading: { levels: [2] } })], content: initial.body, editable: !readOnly, onUpdate: ... , immediatelyRender: false }, [item.id])`; `scheduleSave` 800ms debounce → `updateBinderItemAction(id, { content })` then `updateBinderItem(id, { content })` for optimistic context update; `commitTitle` separate path via `updateBinderItemAction(id, { title })`; "Read-only — your role is Beta Reader." footer when readOnly. `readContent` migration helper handles missing/malformed content (defaults to `{ category: 'OTHER', body: {type:'doc',content:[]}, tags: [] }`). **`chapter-editor.tsx`** — `WikiEntryEditor` import + render-branch `if (activeItem.type === 'wiki_entry') return <WikiEntryEditor item={activeItem} />` added after character branch. **Deviations:** (1) actual `FormSaveStatus` type is `'idle' | 'unsaved' | 'saving' | 'saved'` not plan's `'typing'|'saved'|'error'` — subagent mapped typing→`unsaved`, error→`unsaved` matching character-profile; (2) `contentRef` added to avoid stale-closure tag/category drift between TipTap keystrokes (mirrors character-profile); (3) `onUpdate` guarded with `if (e.isDestroyed) return` per the React 19 TipTap gotcha from `feedback_tiptap_isdestroyed_guard.md`; (4) `updateBinderItemAction` content shape: Zod schema types content as `z.record(z.string(), z.unknown())` — cast through `as unknown as Record<string, unknown>`. tsc clean. Tests 252/252. **Wiki creation flow now end-to-end working in editor:** Add → Wiki Entry ▸ → pick category card → land in editor with pre-seeded body + accent-colored pill + save badge + working tag strip + contenteditable title. Smoke: create entry of each category, type body (badge cycles unsaved→saving→saved), add/remove tags (max 10 enforced, dupes rejected), rename title, switch items + return (content preserved). **T13 (WikiFolderRenderer) COMPLETE** inline in 1 commit: `2fc0f63` — new `wiki-folder-renderer.tsx` (+218 LOC) mirrors `container-view.tsx`'s sheet chrome pattern: theme-aware via `[data-slot="wiki-folder-pane"]` + `[data-editor-theme="light"]` overrides (cool gray canvas dark mode, paper-300 canvas light mode; paper-card children); header has FolderTree icon chip in `--wiki-other` accent + child count metadata + read-only title (renames via binder convention) + SaveStatusBadge. **Editable description**: `<textarea>` with `font-prose` (2 rows, transparent bg, no resize), `placeholder="Describe what lives in this folder…"`, debounced 800ms `scheduleSave` → `updateBinderItemAction(id, { content: { description } })` then optimistic `updateBinderItem(id, { content })`. Children pulled from `useBookEditor.binderItems.filter(i => i.parentId === item.id).sort(by order)`, rendered as `<ul grid auto-fill minmax(240px,1fr)>` of paper-card buttons matching ContainerView's card style (paper bg, brand-yellow border on hover, lift translate). TYPE_META extended to include `wiki_entry` (NotebookPen, --wiki-other) and `wiki_folder` (FolderTree, --wiki-other) for nested rendering. Empty state via `EmptyState` with FolderTree icon when no children — "This wiki folder is empty. Add a wiki entry, character, or sub-folder from the + Add menu in the binder." `chapter-editor.tsx` got the WikiFolderRenderer import + render-branch `if (activeItem.type === 'wiki_folder') return <WikiFolderRenderer item={activeItem} />` placed between wiki_entry and the part/research_folder ContainerView branch. Shipped inline — pattern mirrored ContainerView with minor extensions for description, no judgment calls beyond plan + spec text. tsc clean. Tests 252/252. Smoke: (1) Add Wiki Folder via binder → empty folder with description placeholder + FolderTree empty-state; (2) type description → save badge cycles unsaved → saving → saved; (3) drag wiki_entry into folder → folder shows entry as card; click card → switches to entry editor. **Editor UI phase progress:** T11 (add menu) + T12 (WikiEntryEditor + TagChipStrip) + T13 (WikiFolderRenderer) shipped. Two left in editor UI phase: T14 (outline act grouping) + T15 (character tag strip). Then hive UI (T16 wiki page + T17 outline page) and T18 ship. **Next concrete step:** dispatch H2 T14 (outline editor act grouping — modify `app/[locale]/(app)/studio/[bookId]/_components/outline/outline-board.tsx` to render T6 `groupBeatsByAct` helper output as per-act sections; add per-act header strip with `<input>` for act name + `+ New Act` button at end; beats sortable within and across acts via existing dnd-kit setup; null-act group renders as collapsible "No Act" at top only if any exist; act field is freeform string with autocomplete from `distinctActs` helper. This is load-bearing editor path — needs subagent. Plan section starts line 1848). Then T3 (category templates module) → T4 (requireBinderWritePermission + 50-case truth table) → T5 (tag normalize helper) → T6 (groupBeatsByAct helper) → T7 (wire permission helper into binder write actions + extend getBinderTreeAction projection) → T8 (createHiveAction standalone reshape with shadow book) → T9 (new hive content views: getHiveWikiView/Outline/Notes + new getBinderTreeForHiveAction sibling) → T10 (delete legacy hive_wiki_pages/hive_outlines actions — closes the last tsc cascade) → T11–T15 editor UI (binder add menu grouped, WikiEntryEditor, WikiFolderRenderer, outline acts, character tags) → T16–T17 hive UI (/hive/[hiveId]/wiki real + /outline real) → T18 ship. Full task spec at `docs/superpowers/plans/2026-05-29-h2-mirror-model.md`. After H2: candidates from prior session list still apply — (b) Phase 9 polish (referral codes / growth analytics / plan-upgrade nudges); (c) Stripe dashboard webhook config for live monetization; (d) /settings index page; (e) SP-B Friendships subsystem (separate brainstorm — symmetric request/accept like the old beehive-books-online had); (f) cleanup follow-ups flagged during H1: rename live hive_member_role.BETA_READER → READER to match docs (small T1-style migration); fix inviteLink /en/... locale hardcode in hive-members.tsx; reproject getHiveAction to join userProfiles.username/avatarUrl not users.name/image; delete stale T11 stub create-hive-modal.tsx + dead create-hive-button.tsx under studio/[bookId]/_components/. Outstanding from prior epics: Linked Series + Chapter Status + Drag-into-Folder + Character Redesign + Delete Book + SP-A reader-route checklist still awaiting Chris's smoke (carried from earlier sessions). Per-task T1–T16 narrative trimmed on T17 ship — see What Has Been Built → Hives Redesign — H1 Foundation for the full breakdown.
> **Active branch:** `main`
> **Last commit:** feat(hive): H2 T13 — WikiFolderRenderer
>
