...
>
> **Current focus:** **Corkboard view removed (`8ddf3a9`)** — 5 files changed, 24 insertions, 353 deletions. Dropped the LayoutGrid toggle button (sole entry point) from the binder header, plus all now-unreachable corkboard infrastructure: `corkboardMode` / `toggleCorkboardMode` from `BookEditorProvider` (type + state + value + deps), the conditional render in `CorkboardOrEditor` (always renders `ChapterEditor` now), `corkboardMode ||` from `MetadataPanel`'s hidden gate, the binder tree's `{!corkboardMode && (...)}` wrapping gate, and `corkboard-view.tsx` itself. Binder header's right cluster now has just the Settings cog — balanced. Prior epic: **Delete-Book COMPLETE** (kebab + Danger Zone entry points, sonner toast, DeleteBookButton render-prop, FK cascades). 137/137 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(studio): remove corkboard view + unreachable infrastructure
>
