...
>
> **Current focus:** **Container view redesign shipped (`b9a4a10`)** — the editor surface shown when you select a part or research_folder in the binder. Old: dark `bg-surface` cards on the same `--canvas-dark-100` walnut, clashing with the cream-paper documents the cards open into. New: theme-aware via `[data-slot="container-pane"]` + `[data-editor-theme="light"]` flip pattern (same as character-profile.tsx). Dark mode canvas = lifted neutral dark gray `oklch(0.22 0.005 256)` (less walnut, more pure gray); cards = `--paper-100` cream with 3-layer lift shadow. Light mode canvas = `--paper-300` (darker cream); cards = `--paper-50` (lighter cream). Card polish: display-serif title + mono uppercase type label, 36px brand-accent header chip, hover translates up 1px with brand-yellow border. Prior epic just before this: drag-into-folder (`ff3e711`, after iterative debugging — verticalListSortingStrategy removed, pointerWithin collision detection, separate `:nest` overlay droppable, 87% nest band, parts→chapters only / research_folder→everything-else). 157/157 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(container-view): paper cards on lifted canvas; fix dark-clash
>
