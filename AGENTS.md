...
>
> **Current focus:** **Hives redesign — H1 + H2 + H3 + H4 specs WRITTEN** (commits `7fc7cb7` H1, `2148ab6` H2, `8282b0d` H3, `b742908` milestone-removal amendment to H1+H2+H3, `f1767a9` H4; `docs/superpowers/specs/2026-05-29-h{1,2,3,4}-*.md`). 4 of 5 sub-projects locked; only H5 (Dashboard) remains. H1: one-hive-per-book + standalone allowed; 4 roles (OWNER/MODERATOR/CONTRIBUTOR/BETA_READER); own visibility + `discoverable` boolean; 3 creation paths from /studio; editor binder footer button "Create Hive"/"Go to Hive"; /studio adds Hives section; /community becomes hive-activity feed; /discover/hives filters on `discoverable`; book delete cascades; new `hive_activity` table. H2: single-source-of-truth on `binderItems` — wiki/outline/notes are ONE place. 14 wiki categories via `wiki_entry` + category enum + templates + `wiki_folder`. Outline gains `act` field. Standalone hives via invisible `STANDALONE_HIVE_SHADOW` books rows; tightens H1 to plain UNIQUE on `hives.bookId`. New `requireBinderWritePermission`. Drops `hive_wiki_pages` + `hive_outlines`. H3: collab core. `hiveAnnotations` with 5-layer enum + threaded replies + dual-resolver; position drift via TipTap mark extensions. `hiveSuggestions` reshaped to range-targeted; surgical accept. `hiveSubmissions` reshaped to carry draft TipTap content inline (DRAFT/PENDING/APPROVED/REJECTED); approval creates real chapters with `author_user_id` → reader shows "Written by @submitter". `hive_discussion_posts` gains `topic` enum + CHECK constraint. Drops `hive_chapter_locks`. Writes 8 event types to `hive_activity`. **Mid-brainstorm pivot:** Milestones feature DROPPED from the redesign entirely; H1+H2+H3 specs amended (commit `b742908`) to remove all milestone references. H4: motivation. `hive_word_goals` (DAILY/WEEKLY/MONTHLY/TOTAL, one active per type via partial UNIQUE, auto-archives on new); `hive_word_logs` populated via 60s-throttled hook on `saveChapterAction` (deltas; negatives allowed). `/word-goals` page shows active strip + contributors + recent activity + history; sidebar nav badge shows highest-priority progress. `hive_buzz_posts` (TEXT/LINK only; no images, no comments) + `hive_buzz_likes` with denormalized `like_count`. Writes `buzz_posted` event from same-transaction insert. Permissions: OWNER/MOD set goals; all members post buzz + like. **Awaiting Chris's review of all 4 specs** before deciding: (a) continue to H5 brainstorm (closes the redesign cycle; H5 is the simplest), (b) invoke writing-plans for H1–H4 together, (c) ship per-sub-project. Recommendation: (a). Linked Series + Chapter Status + Drag-into-Folder + Character Redesign + Delete Book all still awaiting Chris's manual smoke (carried from prior session).
> **Active branch:** `main`
> **Last commit:** docs(spec): H4 motivation — word goals + buzz board
>
