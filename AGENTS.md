...
>
> **Current focus:** Executing SP-A reader-route plan via subagent-driven mode ([docs/superpowers/plans/2026-05-28-reader-route-privacy-discoverable.md](docs/superpowers/plans/2026-05-28-reader-route-privacy-discoverable.md)). **Task 1 of 15 shipped** (commit `7e180bb`): `book_visibility` enum extended with FRIENDS, `books.explorable` renamed to `discoverable`, new composite index `books_discoverable_visibility_idx` on `(discoverable, visibility)`, backfill ran against dev DB (0 rows currently match PUBLIC+PUBLISHED). Spec ✅ + Quality ✅ subagent reviews both approved. Migration applied via one-shot tsx script (same trade-off as Phase 7 — `drizzle-kit push` is interactive and fails in non-TTY environments; drizzle snapshot history is out of sync with live DB but that's an accepted repo pattern). Pausing for Chris's per-task verification before Task 2 (canReadBook access helper + unit tests).
> **Active branch:** `main`
> **Last commit:** feat(schema): add FRIENDS visibility, rename explorable->discoverable, backfill
>
