...
>
> **Current focus:** **Hives redesign — H1 + H2 specs WRITTEN** (commits `7fc7cb7` H1 + `2148ab6` H2; `docs/superpowers/specs/2026-05-29-h{1,2}-*.md`). Full Hives redesign decomposed into 5 sub-projects (H1 foundation → H2 mirror → H3 collab core → H4 motivation → H5 dashboard). H1: one-hive-per-book + standalone allowed, 4 roles (OWNER/MODERATOR/CONTRIBUTOR/BETA_READER), hive owns its own visibility + new `discoverable` boolean, 3 creation paths from /studio, editor binder footer button "Create Hive"/"Go to Hive" via `getBookHive(bookId)`, `/studio` gets a new Hives section, `/community` becomes a hive-activity feed (kills `getCommunityFeedAction`), `/discover/hives` filters on `discoverable=true`, book delete cascades to hive. New `hive_activity` table introduced in H1 (H3/H4 wire writes). H2: single-source-of-truth on `binderItems` — wiki/outline/notes live in ONE place, editor and hive are two views over the same rows. 14 wiki categories via one new `wiki_entry` binder type + `category` enum + per-category templates ported from beehive-books-online; plus `wiki_folder` for organization. Outline beats gain optional `act` field. Standalone hives back themselves with invisible `STANDALONE_HIVE_SHADOW` books rows so the single-source model holds; this tightens H1's partial UNIQUE index to plain UNIQUE on `hives.bookId`. Permissions via new `requireBinderWritePermission` helper: BETA_READER read-only on wiki/outline/notes, CONTRIBUTOR+ writable, chapters/parts/FM/BM still author-only (H3 handles chapter changes). Drops `hive_wiki_pages` + `hive_outlines` tables. **Awaiting Chris's review of both specs** before deciding: (a) continue to H3 brainstorm, (b) invoke writing-plans for H1 only, or (c) invoke writing-plans for H1+H2 together. Recommendation: (a) — finish brainstorming H3 to unlock the actual collab payoff (peer-to-peer editing). Linked Series + Chapter Status + Drag-into-Folder + Character Redesign + Delete Book all still awaiting Chris's manual smoke (carried from prior session).
> **Active branch:** `main`
> **Last commit:** docs(spec): H2 mirror model — binder ↔ Hive Wiki/Outline/Notes
>
