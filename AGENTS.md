...
>
> **Current focus:** **Delete-Book COMPLETE** (5 feature commits + plan/spec + sonner install). Two entry points: library card kebab (red Delete item) + `/studio/[bookId]/details` Danger Zone (outlined red Button). Both invoke the same `DeleteBookButton` render-prop wrapper → destructive `ConfirmDialog` → `deleteBookAction(bookId, locale)` → sonner toast + `router.push('/studio') + refresh()`. Hard delete; FK cascades handle everything downstream. See "What Has Been Built" → Delete Book for the full breakdown. **Awaiting Chris's manual verification** (Task 7 checklist): (1) library kebab → Delete → ConfirmDialog → redirects to /studio with toast + card gone; (2) Details Danger Zone → same flow; (3) Cancel from either path leaves book intact; (4) delete a book with chapters/snapshots/comments → /studio loads without orphan errors. 137/137 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(details): Danger Zone with Delete this book
>
