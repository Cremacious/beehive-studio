...
>
> **Current focus:** **Beat-delete confirmation shipped (`1da9c8d`)** — single-file edit (+14/-1) on `outline/outline-card.tsx`. Trash icon now opens a destructive `ConfirmDialog` instead of deleting immediately: "Delete this beat? This removes the beat from your outline, including its summary, status, and any linked chapter reference. This cannot be undone." Confirm button "Delete beat", Cancel returns to the outline untouched. Same ConfirmDialog pattern as binder-item-delete + book-delete — consistent destructive-action vocabulary. Prior: corkboard removal (`8ddf3a9`, -329 lines), Delete-Book epic (kebab + Danger Zone), SP-A reader epic. 137/137 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(outline): confirm before deleting a beat
>
