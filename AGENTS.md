...
>
> **Current focus:** **Chapter-Status Publish-Gate — Tasks 1-5 shipped.** Plan: [docs/superpowers/plans/2026-05-28-chapter-status-publish-gate.md](docs/superpowers/plans/2026-05-28-chapter-status-publish-gate.md). T1 (`34ff8da`): helper + tests. T2+3 (`b73959b`): book reader gates per-row, hero CTA filters. T4 (`7c65b11`): chapter reader page locked placeholder for direct-URL hits. **T5 (`68387a4`)**: binder color dots for chapter rows. `BinderItemRow` widened with `chapterStatus: ChapterStatus | null`; `getBinderTreeAction` projection now includes `chapters.status`; 6px dot renders between icon + title for chapter rows using `--status-*` tokens; provider's `updateChapterStatus` optimistically syncs `chapterStatus` onto `binderItems` + rolls back on failure (live UI update). 3 literal `BinderItemRow` call sites updated (chapter-editor, binder-item-menu, binder-add-menu) — new chapters default `chapterStatus='FIRST_DRAFT'` matching schema default. Spec ✅, quality ✅. **Next:** T6 = metadata panel explainer paragraph + per-pill subtitles ("Visible to readers" / "Not visible to readers"); T7 = AGENTS.md sync; T8 = final verification. 162/162 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(binder): per-chapter status color dot for chapter rows
>
