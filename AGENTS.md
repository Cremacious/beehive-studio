...
>
> **Current focus:** Shipped the Book Details editor — closes the gap where most wizard fields (subgenre, tags, audience, content warnings, comp titles, language, series, subtitle, …) had no edit path post-creation. New route `/[locale]/studio/[bookId]/details` is a server page + client form with four collapsible sections (Basics / Discovery / Structure / Publishing). Save model is explicit "Save changes" button with dirty-state tracking. Three entry points wired: Settings cog in the binder header (next to the corkboard toggle), kebab menu on the library book card with "Open" + "Edit details" (card was refactored from a single Link to a div + sibling button to allow nested interactive elements), and a small footer hint at the bottom of the /studio/new wizard. Backend: new `getBookDetailsAction` + `updateBookDetailsAction` (transactional, writes to both `books` and `bookPublishingMetadata.subtitle`); new `updateBookDetailsSchema` covers every wizard field. Premium-gated publishing fields (publisher/trim/edition/isbn/authorBio/dedication) are read-only in the form with an Upgrade chip — they're still editable via the existing in-editor Publishing expander on Premium. Template field is intentionally excluded (it was a one-time binder seed, not a setting). Hive-integration spec remains locked at [docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md](docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md). 126/126 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(studio): book details editor at /studio/[bookId]/details
>
