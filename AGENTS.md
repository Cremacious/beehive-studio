...
>
> **Current focus:** **Binder drag-anywhere shipped (`ab639f1`)** — single-file edit (+13/-5) on `binder/binder-item.tsx`. Moved dnd-kit's `{...attributes} {...listeners}` from the GripVertical span to the outer row `<div>` so any part of a binder row is grabbable. `cursor-grab` on row when not renaming, `cursor-text` while renaming. Listeners suppressed during `isRenaming` so text-selection in the title input doesn't trip the 8px sortable activation threshold. Grip icon stays as hover-only visual hint (`aria-hidden` now). `aria-label="Drag to reorder"` moved up to the row. PointerSensor's existing 8px distance constraint cleanly separates clicks (select item) from drags (reorder); collapse chevron and kebab menu still receive clicks unaffected. Prior: character profile redesign (`8ec4eca`, specimen-sheet), beat-delete confirmation, corkboard removal, Delete-Book epic, SP-A reader epic. 137/137 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(binder): whole row drags, not just the grip
>
