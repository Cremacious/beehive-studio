...
>
> **Current focus:** Fixed binder activation flow end-to-end. Two bugs: (1) `setActiveItemId` read `binderItems` from a stale closure → never switched the editor when called in the same tick as `addBinderItem`; fix added a `binderItemsRef` mirror. (2) Regression from the first fix: the ref only updated on render commit, so chapter creation got stuck on the loading skeleton because the same-tick fetch branch couldn't see the new item. Final fix: `addBinderItem` now updates `binderItemsRef.current` synchronously before queueing `setBinderItems`, so the next call in the same tick reads the fresh array and the chapter fetch fires immediately. Also added `ContainerView` renderer for `part` (Collection) and `research_folder` types — they now show a clickable child grid + empty state instead of a blank viewport. Hive-integration spec remains locked at [docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md](docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md). 126/126 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** fix(studio): sync binderItemsRef in addBinderItem (chapter loading regression)
>
