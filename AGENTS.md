...
>
> **Current focus:** Executing SP-A reader-route plan via subagent-driven mode ([docs/superpowers/plans/2026-05-28-reader-route-privacy-discoverable.md](docs/superpowers/plans/2026-05-28-reader-route-privacy-discoverable.md)). **Tasks 1-13 of 15 shipped, awaiting decision on T13 follow-up.** Prior tasks: T1-T12 shipped clean (see prior commits). T13 `c1b1817`: binder header book title wrapped in `<Link>` to `/${locale}/books/${bookId}` with onDoubleClick preserving rename via `e.preventDefault()`; aria-label + `no-underline` + `hover:underline` added; original h2 className preserved. Spec ✅ but **Quality ⚠️ flagged TWO MINOR issues**: (1) **dual-click race** — single click fires SPA navigation before the browser resolves it as dblclick; `e.preventDefault()` only suppresses the *second* click, the first one has already navigated, so users trying to rename will frequently land on the reader page first; (2) **semantic regression** — the `<h2>` became `<a>`, losing heading semantics. Both real UX problems. Asked Chris to pick between **A** (wrap Link in h2 + debounce single-click 250ms), **B-recommended** (wrap Link in h2 + move rename to the kebab/⋯ menu so title is pure-navigation — matches other binder-item rename UX), or **C** (ship as-is, internal author-only surface). 137/137 tests, tsc clean. Spec ✅ on all 13; Quality ✅ on 1-12, ⚠️ on 13. **AWAITING CHRIS'S DECISION on A/B/C before continuing.**
> **Active branch:** `main`
> **Last commit:** feat(binder): book title in header links to reader
>
...
>
> **Next concrete step when resuming:** Resolve T13's two MINOR quality concerns by Chris's pick. **Option A:** Wrap the new `<Link>` in `<h2>` (restore heading semantics) + debounce single-click 250ms cancelled on dblclick (covers the race). **Option B (recommended):** Wrap in `<h2>` + move rename to the binder-tree's existing kebab/⋯ menu (matches how other binder items rename, makes title pure-navigation, removes race entirely). **Option C:** Ship as-is. After resolving T13, dispatch Task 14 (update AGENTS.md "What Has Been Built" section to add an "SP-A — Reader Route + Privacy/Discoverable" entry summarizing the shipped work) then Task 15 (`npm test && npx tsc --noEmit` final pass + Chris's manual verification checklist from the plan). After each task: dispatch implementer → spec reviewer → quality reviewer → checkpoint with Chris before next task (per-task manual verification preference). The spec and plan are locked — do not re-litigate without explicit instruction. Subagent flow is documented in `C:/Users/Chris/.claude/plugins/superpowers/skills/subagent-driven-development/`. After SP-A ships, the next decisions in order are: (a) SP-B Friendships subsystem (separate brainstorm — symmetric request/accept like the old beehive-books-online had); (b) Hive ↔ Binder integration via the locked spec at [docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md](docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md); (c) Phase 9 polish (referral codes, growth analytics, plan-upgrade nudges); (d) Stripe dashboard webhook configuration for live monetization; (e) a `/settings` index page.
