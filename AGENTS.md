...
>
> **Current focus:** **Back-matter TipTap mini-editor affordance shipped (`b5cf279`)** — extends the previous FM/BM affordance fix to the two subtypes that use TipTap mini-editors instead of contenteditable `.bp-field`s: Acknowledgments and About Author. Both previews now configure `@tiptap/extension-placeholder` with subtype-specific copy. `page-wrapper.tsx` CSS adds matching rules: `.bp-body.is-editor-empty` gets the same dashed outline as `.bp-field:empty`; TipTap's `.is-editor-empty:first-child::before` pattern renders the placeholder text in paper-ink-muted italic at 75% opacity. I-beam cursor on hover. Same visual language as `35af098` so the two surfaces feel consistent across all 5 FM/BM subtypes. **Pattern note:** `<style>` blocks inside JSX are template literals — backticks for inline-code (e.g., \`.is-editor-empty\`) inside CSS comments will be parsed as nested template literals and break compilation. Use bare class names in comments instead. Recent arc: (`35af098`) FM/BM dashed affordance on empty `.bp-field`s; (`dc9f08d`) note toolbar below surface-head; (`f240c82`) notes use full EditorToolbar; (`cc508fb`) note paper IS the canvas; (`b9a4a10`) container view paper cards on lifted gray; (`ff3e711`) drag-into-folder. 157/157 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(back-matter): add input affordance to TipTap mini-editors (Acknowledgments, About Author)
>
