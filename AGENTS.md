...
>
> **Current focus:** **SP-A — Reader Route + Privacy/Discoverable COMPLETE** (15/15 tasks shipped + 1 follow-up). End-to-end: studio editor → toolbar Preview (Eye icon) OR binder-header title click → `/[locale]/books/[bookId]` reader (canReadBook-gated) → chapter reader at `/read/[chapterId]` with mark-as-read. **Follow-up shipped (`67c3a39`):** third entry point added — "Preview" item (Eye icon) in the book-card kebab menu in `/studio`, between Open and Edit details. Closes the gap where reaching the reader from the library required Open → editor → toolbar Eye (two clicks instead of one). Privacy fully threaded: wizard Sharing step on create, Sharing section on `/studio/[bookId]/details` for ongoing changes; PRIVATE/FRIENDS/PUBLIC + discoverable toggle. FRIENDS visibility settable but resolves to author-only until SP-B Friendships ships. Old `/discover/book/*` URLs 308-redirect to canonical `/books/*`. T13 shipped with two known MINOR quality concerns ("ship as-is" per Chris): dual-click race on binder title (single click may navigate before dblclick resolves to rename) and h2→a semantic regression — defer to follow-up if it bites. 137/137 tests, tsc clean across the run.
> **Active branch:** `main`
> **Last commit:** feat(studio): Preview item in book card kebab menu
>
