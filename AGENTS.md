...
>
> **Current focus:** **Hives redesign — H1 EXECUTION IN PROGRESS (16 of 17 tasks shipped — server layer + /studio Hives UI + CreateHiveModal + BinderHiveFooter + /community rewrite + /discover swap-verify + /hive shell all complete; only T17 ship-commit remains).** Subagent-driven execution flow: dispatch implementer → spec reviewer → code-quality reviewer → checkpoint with Chris before next task. Plan at `docs/superpowers/plans/2026-05-29-h1-hive-foundation.md`. **Task 1 (schema migration) COMPLETE** in 3 commits: (1) `05bdf3b` — `db/schema/hive.ts` + `scripts/migrate-h1.ts`: enum collapse 5→4 (EDITOR→MODERATOR, PROOFREADER→CONTRIBUTOR), `hives.discoverable boolean`, partial UNIQUE `hives_book_id_unique WHERE book_id IS NOT NULL` (H2 will tighten — intentional), `hives.book_id` FK→CASCADE, new `hive_activity` table + 10-value `hive_activity_type` enum. Plus minimal spillover patches in `_helpers.ts` + `hive.actions.ts` for EDITOR/PROOFREADER → MODERATOR string rewrites to keep tsc clean. (2) `570082f` — fix orphaned FK from migration; live DB now has exactly ONE `hives.book_id` FK (CASCADE); script idempotent via `DROP CONSTRAINT IF EXISTS` for both possible names. (3) `8b16a8b` — wrap `CREATE TYPE hive_member_role_new` in `DO $$ EXCEPTION WHEN duplicate_object` for swap-dance partial-failure re-runnability + align H1 spec to 10-value activity enum (originally listed 7, plan/code use 10 for H3 forward-compat). Live DB verified: discoverable column present, partial UNIQUE intact, CASCADE FK, role enum has 4 values, hive_activity table created, tsc clean. **Tasks 2–10 (server layer) COMPLETE** in 9 commits: `a759f10` T2 permission helpers (4 roles × 8 predicates + 3 require-* helpers, 8/8 tests), `81eb945` T3 `getBookHive` reverse-lookup helper with React `cache()` memoization (2/2 tests), `799a567` T4 `recordHiveActivity` writer + `getHiveActivityFeedAction` member-scoped cursor-paginated feed (1 test, joins `userProfiles.username`/`avatarUrl` not `users.name`/`image` — project convention), `fe907f0` T5 `createHiveAction` reshape with 3 paths (bookId/null/standalone), free-tier FREE_HIVE_LIMIT check, uniqueness via `getBookHive`, OWNER membership in same transaction (4/4 schema tests; 186 total suite), `6238f04` T6 `updateHiveAction` reshape with discoverable→false coercion when visibility≠PUBLIC, fixed stale `HiveMemberRow` type (was listing EDITOR/PROOFREADER), deleted `getUserHivesAction` + `getMyHivesAction` (note: `getMyHivesAction` was in `hive.actions.ts` not `community.actions.ts` — plan pre-flight wrong), `8bc82cc` T7 `getUserHivesView` projection via raw SQL `db.execute` (returns id/name/desc/bookId/bookTitle/bookCoverUrl/visibility/discoverable/status/memberCount/lastActiveAt/viewerRole; orders by COALESCE(MAX hive_activity.created_at, hives.created_at) DESC), `867cbc9` T8 emits `member_joined` activity row after `acceptHiveInviteAction` transaction commits (Option B durability: activity write outside tx so failure doesn't void membership), `cbb7adc` T9 renamed `getPublicHivesAction` → `getDiscoverableHivesAction` with filter tightened to `visibility='PUBLIC' AND discoverable=true` (function lived in `hive.actions.ts` not `discover.actions.ts` — plan path was wrong), `bc2c164` T10 deleted `getCommunityFeedAction` + helpers from `community.actions.ts`; `app/[locale]/(app)/community/page.tsx` + `feed-list.tsx` stubbed with empty feed + no-op loadMore + TODO H1-T14 markers to keep tsc clean until T14 rewrites the page. tsc clean throughout. Remaining tasks 11–17 deferred to next session: T11 `/studio` Hives section UI + HiveCard, T4 recordHiveActivity + getHiveActivityFeedAction, T5–T10 server actions (createHive 3 paths, updateHive coercion, getUserHivesView, member_joined wiring, discover rename, delete getCommunityFeedAction), T11–T14 UI (/studio Hives section, CreateHiveModal, BinderHiveFooter, /community rewrite), T15 /discover swap confirm, T16 /hive Settings+Members+stubs, T17 ship commit. **Tasks 11–12 (UI shell) COMPLETE** in 3 commits: `5540c9c` T11 `/studio` Hives section (server page parallel-fetches `getUserHivesView` alongside books; new `HivesSection` client component with search/sort/filter-chips Owned/Member + Linked/Standalone-with-hasBoth-guard + grid + empty states; new `HiveCard` with cover or honeycomb-SVG fallback + role pill via --status-* tokens + member count subline + last-active label or "No activity yet"; cards link to `/{locale}/hive/{id}`), `d805600` follow-up fix (StudioEmptyState was early-returning when books.length===0 even if user had hives — now requires BOTH empty; recentBook + BookGrid render conditionally), `045b339` T12 `CreateHiveModal` real implementation (replaced T11 stub) — two-step: 3-radio path picker (Link existing book / Create new book + hive / Standalone) then details form (name/desc/visibility/discoverable with auto-clear when visibility≠PUBLIC); `lib/actions/book.actions.ts` extended with leftJoin to `hives` + `hiveId` in BookSummary projection (used by modal's eligible-books dropdown); `/studio/new` wizard's `book-creation-form.tsx` reads `?withHive=1` and redirects to `/studio?createHive=<newBookId>` after book creation; `HivesSection` reads `?createHive=` query param via useSearchParams + useEffect + router.replace to strip param, auto-opens modal pre-locked. tsc clean. **Task 13 (BinderHiveFooter reshape) COMPLETE** in 1 commit: `a4f1882` — studio book page (`app/[locale]/(app)/studio/[bookId]/page.tsx`) now calls `getBookHive(bookId)` in parallel with existing loads and passes `bookHive: { hiveId } | null` to `BookEditorProvider`; provider extended with `bookHive` in context type + props (default null) + memo deps; `binder-hive-footer.tsx` branches on `bookHive` — Users icon + `<Link>` to `/${locale}/hive/${hiveId}` with "Go to Hive" when set, otherwise Plus icon button + `CreateHiveModal` flow with "Create Hive". Bonus: pre-existing footer was importing the obsolete T11 *stub* modal at `[bookId]/_components/create-hive-modal.tsx` with stale `bookId`/`locale`/`onClose` API; T13 swapped it to the real T12 modal at `studio/_components/create-hive-modal.tsx` with `open`/`onOpenChange`/`prelockBookId` — un-bricks the existing Create-Hive button. Stale stub modal + dead `create-hive-button.tsx` (no callers) still on disk; out of scope. tsc clean. Awaiting Chris's manual smoke: (1) book with no hive shows "Create Hive" → modal opens prelocked → submit → reload shows "Go to Hive" linking to `/{locale}/hive/{hiveId}`. **Task 14 (/community rewrite as hive-activity feed) COMPLETE** in 1 commit: `7bc4d43` — `app/[locale]/(app)/community/page.tsx` rewritten with 4-way parallel `Promise.all` (`getHiveActivityFeedAction({ limit: 30 })`, `getSuggestedWritersAction`, `getMyActiveSparksAction`, `getUserHivesView`) + inline 2-col grid layout (`1fr_280px`); new `ActivityFeed` client component (useTransition for Load older button, `useParams<{locale}>` for hive links, empty state "You're not in any hives yet" → `/{locale}/studio`); new `ActivityEventRow` with per-type copy map covering all 10 `hive_activity_type` enum values (member_joined / chapter_submitted / chapter_submitted_approved / chapter_submitted_rejected / discussion_posted / annotation_added / suggestion_proposed / suggestion_accepted / suggestion_rejected / buzz_posted), avatar + actor profile link + hive link + inline `relTime` helper (date-fns not installed, matched existing codebase pattern); `MyHivesPanel` retyped to `UserHiveView[]` from `hive.actions.ts` with "View all" → `/{locale}/studio` (Hives section lives there post-T11). Deleted 4 stale follows-feed files: `feed-list.tsx`, `feed-item.tsx`, `community-page-shell.tsx`, `suggested-writers-strip.tsx`. Net −155 lines. SuggestedWritersPanel + ActiveSparksPanel mounted as-is (no internals touched). tsc clean. Awaiting Chris's manual smoke: (1) zero-hives user → empty state w/ studio CTA; (2) join hive, trigger events → /community shows feed with right copy + links; (3) >30 events → Load older appends; (4) sidebar MyHives shows up to 5 hives w/ member counts + links. **Task 15 (`getPublicHivesAction` → `getDiscoverableHivesAction` swap verification) COMPLETE** — no commit (zero code change, T9 covered all callers). Grep confirms zero stale `getPublicHivesAction` references; `getDiscoverableHivesAction` has 2 call sites (definition in `lib/actions/hive.actions.ts` + consumer at `app/[locale]/(public)/discover/page.tsx`). Awaiting smoke: hive w/ visibility=PUBLIC + discoverable=true appears on /discover Hives tab; toggling discoverable=false removes it but URL still works for members. **Task 16 (`/hive/[hiveId]` shell — sidebar + Settings + Members + 8 stubs) COMPLETE** in 1 commit: `d162ddf` — rewrote `hive-sidebar.tsx` with 11-entry NAV (Dashboard/Outline/Wiki/Annotations/Discussions/Submit Chapter/Edit Suggestions/Word Goals/Buzz Board/Members/Settings) using lucide icons + brand-yellow active marker; simplified landing `page.tsx` (Welcome to {hive} + member count + last-active relTime); new `settings/page.tsx` with OWNER guard (non-OWNER inline message, no redirect) + new `_components/hive-settings-form.tsx` (name/desc/3-card visibility picker via SharingControls-style/discoverable w/ 3-layer defense, Danger Zone delete via shared `ConfirmDialog` from delete-book pattern, sonner success/error toasts, post-delete redirect to `/{locale}/studio`); revamped `hive-members.tsx` with OWNER-only role dropdown wired to `updateMemberRoleAction` (optimistic-with-rollback) + sonner toasts + remove button rollback; 8 new stub routes (`annotations/`, `discussions/`, `submissions/`, `suggestions/` → "Coming in H3 (Collaboration)"; `word-goals/`, `buzz/` → "Coming in H4 (Engagement)"; `outline/`, `wiki/` repurposed to "Coming in H2 (Mirror model)") via shared `_components/coming-soon.tsx`. Deleted ~7 P7.5 scaffolding files: `binder/page.tsx` + `hive-binder.tsx`, `tasks/page.tsx` + `hive-tasks.tsx`, singular `discussion/page.tsx` + `hive-discussion.tsx`, `wiki/[pageId]/page.tsx` + `hive-wiki.tsx`, `hive-outline-editor.tsx`, `hive-overview.tsx`, `hive-chapter-comments.tsx`. Net −367 lines (410 added / 777 deleted across 26 files). Implementer flags: (a) role enum drift — plan text said `READER` but live schema uses `BETA_READER`; implementer aligned to reality (4 values now OWNER/MODERATOR/CONTRIBUTOR/BETA_READER not the documented OWNER/MODERATOR/CONTRIBUTOR/READER from T1 commit msg — schema audit needed if doc should win); (b) `inviteLink` in `hive-members.tsx` hardcodes `/en/...` locale (pre-existing bug, untouched); (c) `getHiveAction` projection still joins `users.name/image` not `userProfiles.username/avatarUrl` (inconsistent with H1 convention used elsewhere, left as-is). tsc clean. Awaiting Chris's manual smoke: (1) 11-entry sidebar renders w/ active state; (2) 8 stubs say "Coming in HX"; (3) Settings as OWNER edits+saves persist; visibility flip clears+disables discoverable; (4) Settings Delete via ConfirmDialog → redirect to /studio; (5) Settings as non-OWNER → "owner only" message; (6) Members OWNER role dropdown + remove + rollback on server error; (7) Members non-OWNER no dropdowns; (8) non-member blocked at layout. **Next concrete step:** dispatch Task 17 (FINAL — AGENTS.md "What Has Been Built" entry for "Hives Redesign — H1 Foundation ✅ COMPLETE (2026-05-29)" following format of prior phase entries; plan section starts line 1736 with the exact entry template; close out the Resume Here block by moving H1 from "in progress" to historical complete + flip Current focus to next-thing-TBD; final commit `feat: H1 Hive Foundation complete — sidebar + Settings + Members + stubs across /studio, /community, /hive routes`). T14 rewrites `/community` as hive-activity feed using `getHiveActivityFeedAction` (replaces T10 stubs; sidebar keeps Phase-7.5 panels). T15 is just a verification that T9's rename took (likely no code change needed). T16 lands `/hive/[hiveId]` shell with Settings + Members real pages + 8 H2-H5 "Coming soon" stubs. T17 ships AGENTS.md + final commit. UI tasks continue full review-subagent cycle. Exact code shapes in plan at `docs/superpowers/plans/2026-05-29-h1-hive-foundation.md`. Full Hives redesign spec stack (H1–H5) committed earlier: `7fc7cb7` H1, `2148ab6` H2, `8282b0d` H3, `b742908` milestone-removal amendment, `f1767a9` H4, `d11db57` H5. Linked Series + Chapter Status + Drag-into-Folder + Character Redesign + Delete Book all still awaiting Chris's manual smoke (carried from prior session).
> **Active branch:** `main`
> **Last commit:** feat(hive): sidebar nav + Settings + Members + H2-H4 stub pages
>
