...
>
> **Current focus:** **Linked Series execution — Tasks 1-5 shipped.** Plan: [docs/superpowers/plans/2026-05-28-linked-series.md](docs/superpowers/plans/2026-05-28-linked-series.md). T1 (`9849971`): `normalizeSeriesKey` helper + 7 tests. T2 (`d99bc70`): `getSeriesNeighbors` server helper + 6 tests. T3 (`2142f3a`): BookSummary projection extension. T4 (`d5bcec1`): SeriesLine + 3 cards. **T5 (`f207e6f`)**: library "By series" sort + clustering. Added to `book-grid.tsx`'s SORT_LABELS; cluster builder uses `normalizeSeriesKey` to group, first-encountered raw `seriesName` provides cluster displayName, within-cluster sort is `seriesNumber` asc with nulls last, "Standalone (N)" trailing cluster renders only when non-empty. Operates on already-filtered `visible` list so filter chips remain accurate. Filter chip counts derived from raw books array unchanged. Cluster order = insertion order (not alphabetical) — minor non-blocking concern flagged if Chris prefers alphabetical-by-series-name. Spec ✅, quality ✅. **Next:** T6 = book reader hero series line + `<SeriesFooter>` prev/next; T7 = wizard + Details explainer copy; T8 = AGENTS.md sync + final verification. 175/175 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(library): 'By series' sort clusters books with series subheaders
>
