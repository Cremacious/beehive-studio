...
>
> **Current focus:** Fixed long-standing binder activation bug — `setActiveItemId` was reading `binderItems` from a stale closure, so calling it in the same tick as `addBinderItem` (the create-and-open path in `BinderAddMenu` + `EmptyStartChapter`) would silently bail on the `.find()` lookup. The new item appeared "selected" (the rename input is on a separate `pendingRenameId` path) but the editor never switched — edits silently went to the previously-open chapter. Fix adds a `binderItemsRef` mirror (same pattern as `chapterCacheRef`); `setActiveItemId` now reads from the ref and always calls `setActiveItemIdState(id)`. Removed `binderItems` from the callback deps so it's stable. Also added a `ContainerView` renderer for `part` (Collection) and `research_folder` types — they now show a clickable child grid + empty state instead of a blank viewport. Hive-integration spec remains locked at [docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md](docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md). 126/126 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** fix(studio): editor opens newly created binder items
>
