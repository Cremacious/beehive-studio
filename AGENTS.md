...
>
> **Current focus:** **Character profile redesign — spec written, awaiting Chris's review before plan-phase.** Spec at [docs/superpowers/specs/2026-05-28-character-profile-redesign-design.md](docs/superpowers/specs/2026-05-28-character-profile-redesign-design.md) (`29564e8`). Single-file presentational rewrite of `character-profile.tsx`: collapse the 2-column section grid (which caused the Appearance↔Personality row-stretch bug) into a single continuous "specimen sheet" with sections separated by thin horizontal rules; drop per-section card chrome (background/shadow/radius) so each section grows independently. Typography refinements: section heading 17→16px, body line-height 1.65→1.7. Identity header card preserved; logic (legacy migration, debounce, save badge, theme tokens) untouched. No DB/schema/test changes. Prior: beat-delete confirmation (`1da9c8d`), corkboard removal (`8ddf3a9`), Delete-Book epic, SP-A reader epic. 137/137 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** docs(character): spec for single-column specimen-sheet redesign
>
