...
>
> **Current focus:** **Drag-into-folder COMPLETE + collision bug fixed (`446bb6f`)**. Chris reported "unable to drag and drop documents into either a collection or research folder" — root cause was a dnd-kit collision-detection issue: `setNodeRef` (droppable handle) was on the outer wrapper that contained the row PLUS its rendered children, so an expanded folder's ref'd rect spanned the whole subtree. `closestCenter` then sank into the children area and a child row always stole the `over` event, making the folder itself unreachable as a drop target. Fix: moved `setNodeRef` to just the row; children render as siblings of the row inside a plain layout wrapper that dnd-kit doesn't see; `paddingLeft` merged into the row's combined style spread alongside the transform. Each child's own setNodeRef stays wired via its own `useSortable`, so SortableContext order is unchanged. **Pattern note:** in dnd-kit nested sortables, NEVER wrap a setNodeRef'd element around rendered descendants — the droppable rect must match the visual click target. Prior tasks: T1 `32c9626` (pure helpers + 17 unit tests), T2 `bf33c31` (binder-tree wiring), T3 `eadea69` (indicators); see "What Has Been Built" → Drag-into-Folder. **Awaiting Chris's re-smoke** of the drop-onto-folder paths. 154/154 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** fix(binder): move setNodeRef to row only so folders can be drop targets
>
...
- **Accept rules** (`lib/binder/drop-rules.ts`): `part` accepts `chapter` + `part`; `research_folder` accepts `research_note` + `research_folder`; other types are top-level only. Cycle guard walks `parentId` upward from the target — rejects if active.id appears in the chain (with a `seen` Set defense against corrupt data). Pure helpers (`getAcceptedChildTypes`, `canNest`, `classifyDropZone`) unit-tested in isolation (17 tests in `lib/binder/__tests__/drop-rules.test.ts`).
- **Three drop zones per row** (`classifyDropZone`): folders get top 6px = reorder before, middle = nest into, bottom 6px = reorder after. Non-folder rows split at vertical midpoint (before / after only). Zone classified during `onDragOver` from the dragged ghost's center Y (`active.rect.current.translated.top + height/2`) relative to the over row's rect.
- **Visual indicators** (`binder-item.tsx`): thin brand-yellow horizontal line above/below the row for before/after; `ring-2 ring-brand bg-brand/10` on the folder row for nest. Rendered via a new `dropZone: { overId, zone } | null` field on the existing `BinderTreeContext`.
- **Auto-expand** (`binder-tree.tsx`): middle-body hover on a collapsed folder for 500ms expands it. Implemented with two `useRef`s (timer + target id) so re-hovering the same folder doesn't reset the countdown. Timer canceled on drag end, on invalid hover, and on any guard-return path inside `handleDragOver`. The "already expanded" check inside the timer callback prevents redundant state writes.
- **Drop behavior** (`handleDragEnd` in `binder-tree.tsx`): branches on the captured `dropZone`. `middle` → `parentId = folderId, order = maxOrderInFolder + 1` (no sibling reshuffle). `before`/`after` → adopts `overItem.parentId`, splices at the correct insertion index, recomputes orders. All updates flow through the existing `reorderBinderItemsAction` — no server-action changes (pre-flight confirmed it accepts arbitrary `{id, order, parentId}` tuples).

No DB / schema changes. 154/154 tests (137 baseline + 17 new), tsc clean. Two non-blocking concerns noted: timer cleanup on unmount (low risk; would require `useEffect` cleanup hook), and `.find()` perf on very large binders (Map lookup would be cheaper). Both deferred.
