...
>
> **Current focus:** **Back-matter dashed outline fix (`b315563`)** — `b5cf279` added placeholder text + outline rules for TipTap mini-editor bodies but the outline never appeared. Root cause: `.bp-body.is-editor-empty` compound selector — `.bp-body` is on the `.ProseMirror` editor root, but TipTap's `.is-editor-empty` class lives on the first empty PARAGRAPH (per `@tiptap/extension-placeholder`). The two classes were never on the same element. Fixed with `.bp-body:has(p.is-editor-empty)` to lift the empty-state up to the wrapper. Placeholder selector tightened to `.bp-body p.is-editor-empty:first-child::before`. Acknowledgments + About Author bodies now show the same dashed outline as front-matter `.bp-field`s. **Pattern note for future TipTap work:** `is-editor-empty` is on the first paragraph, not the editor root — use `:has()` when you need to scope styles to the wrapper. Recent arc: (`b5cf279`) back-matter TipTap affordance attempt; (`35af098`) FM/BM dashed affordance on `.bp-field`s; (`dc9f08d`) note toolbar below surface-head; (`f240c82`) notes use full EditorToolbar; (`cc508fb`) note paper IS the canvas; (`b9a4a10`) container view paper cards on lifted gray; (`ff3e711`) drag-into-folder. 157/157 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** fix(back-matter): dashed outline on empty TipTap bodies (use :has)
>
