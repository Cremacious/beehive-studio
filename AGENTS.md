...
>
> **Current focus:** **Drag-into-folder COMPLETE + actually working (`ff3e711`)**. The original implementation looked correct on paper but didn't work in practice. Three things were wrong, all fixed: (1) `verticalListSortingStrategy` made folder rows vacate when hovered, so "drop ON folder" was never possible — removed the strategy from SortableContext; (2) `closestCenter` compared the dragged GHOST's center (offset from the user's pointer by their grab offset) to candidate centers, so `over.id` resolved to a neighbor instead of the folder under the pointer — replaced with `pointerWithin → rectIntersection` composition using real pointer Y (tracked via a global `pointermove` listener); (3) "drop ON a sortable item" was ambiguous (reorder vs nest), so folder rows now expose a SEPARATE `useDroppable` overlay with id `${node.id}:nest` covering the row's middle band — the overlay is NOT in SortableContext so it doesn't trigger sortable reorder, and custom collision detection prefers `:nest` ids. handleDragOver/handleDragEnd strip the suffix to get the logical folder id. **Ergonomics:** nest band 87% of row (3px edges from 6px); indicator beefed with `bg-brand/20` + 4px brand halo. **UX rule change:** `part` accepts only `chapter` (was chapter+part); `research_folder` accepts every non-chapter type (research_note, research_folder, character, outline, front_matter, back_matter). Prior commits in this arc: `32c9626` (drop-rules helpers + tests), `bf33c31` (binder-tree wiring), `eadea69` (indicators), `446bb6f` (setNodeRef on row only). 157/157 tests (was 154; +3 net), tsc clean.
> **Active branch:** `main`
> **Last commit:** fix(binder): make drag-into-folder actually work, then make it easy
>
