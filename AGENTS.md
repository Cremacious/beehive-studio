...
>
> **Current focus:** **Linked Series execution — Task 1 of 8 shipped (`9849971`).** Plan: [docs/superpowers/plans/2026-05-28-linked-series.md](docs/superpowers/plans/2026-05-28-linked-series.md). T1: `lib/books/series-key.ts` exports `normalizeSeriesKey(name)` — lowercase, strips leading "The ", collapses whitespace, returns null for null/empty/whitespace-only. 7 unit tests. Spec ✅, quality ✅. **Next:** T2 = `getSeriesNeighbors` server helper + 6 tests (mocked DB + canReadBook); T3 = extend `BookSummary` projection with seriesName/seriesNumber; T4 = shared `SeriesLine` component for library/discover/profile cards; T5 = library "By series" sort + clustering; T6 = book reader hero series line + prev/next footer; T7 = wizard + Details explainer copy; T8 = AGENTS.md sync + final verification. 169/169 tests (162 baseline + 7 new), tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(books): normalizeSeriesKey helper + tests
>
