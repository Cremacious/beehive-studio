...
>
> **Current focus:** **Note editor redesign shipped (`cc508fb`)** — paper IS the canvas. Old: 600px cream card floating on a dark canvas (`var(--background)`) with a heavy drop shadow; the dark margins on either side felt heavy and the note read as a postcard pinned to a wall rather than a writing surface. New: pane background is `var(--paper-100)` directly, prose sits on paper without a separate card wrapper. Matches chapter editor pattern (prose IS the canvas). Container 600→720px, title 28→32px, body 16.5→17px. `data-slot="note-card"` selector survives so the existing ProseMirror prose styles still scope correctly. Notes still render as paper regardless of editor theme (physical-note design intent). Just before this: container view redesign (`b9a4a10`) — same paper-on-lifted-gray pattern for the folder/collection grid surface. Just before that: drag-into-folder shipped (`ff3e711`, after iterative debugging). 157/157 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(note-editor): paper IS the canvas; drop the floating-card chrome
>
