...
>
> **Current focus:** **Drag-into-folder COMPLETE** (5/5 tasks shipped across 3 feature commits + drop-rules helpers + docs). T1 `32c9626` (pure helpers + 17 unit tests), T2 `bf33c31` (binder-tree wiring), T3 `eadea69` (binder-item indicators). See "What Has Been Built" → Drag-into-Folder for the full breakdown. **Awaiting Chris's manual smoke**: drag chapter onto middle of part → nests; onto top/bottom edge → reorders before/after; drag character onto part → no indicator (type mismatch); drag part onto its own descendant → no indicator (cycle); hover collapsed folder middle ~500ms → auto-expands; drag chapter out of part by dropping between top-level items → un-nests via existing flow. Prior: character redesign, beat-delete confirmation, corkboard removal, Delete-Book epic, SP-A reader epic. 154/154 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(binder): render drop indicators (before/middle/after)
>
