...
>
> **Current focus:** **Chapter-Status Publish-Gate execution — Task 1 of 8 shipped (`34ff8da`).** Plan: [docs/superpowers/plans/2026-05-28-chapter-status-publish-gate.md](docs/superpowers/plans/2026-05-28-chapter-status-publish-gate.md). T1 created `lib/books/is-chapter-reader-visible.ts` + 5 unit tests. Helper exports `ChapterStatus` union (mirroring the `chapter_status` pgEnum) and `isChapterReaderVisible(status)` — returns true for REVISED/FINAL, false otherwise. Pure function, no deps. Spec ✅, quality ✅. **Next:** T2+3 (combined per plan recommendation, tightly coupled) — thread chapter `status`+`updatedAt` through the `/books/[bookId]` page reader query + `isAuthor` flag → widen `ChapterList` props + gate non-author + non-visible chapters with "Draft — coming soon" badge + render "Updated MMM DD, YYYY" label for visible chapters. T4 = chapter reader page locked placeholder; T5 = binder color dots; T6 = metadata panel explainer + per-pill subtitles; T7 = AGENTS.md sync; T8 = final verification. 162/162 tests (157 baseline + 5 new), tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(books): isChapterReaderVisible helper + tests
>
