...
>
> **Current focus:** Executing SP-A reader-route plan via subagent-driven mode ([docs/superpowers/plans/2026-05-28-reader-route-privacy-discoverable.md](docs/superpowers/plans/2026-05-28-reader-route-privacy-discoverable.md)). **Tasks 1-3 of 15 shipped.** T1 (`7e180bb`): schema — FRIENDS enum, explorable→discoverable, composite index, backfill. T2 (`acf7948`): `canReadBook` helper + 6 unit tests. T3 (`a8567ce`): discover-listing refactor — `getDiscoverFeedAction` and `getDiscoverWritersAction` now filter on `discoverable=true AND visibility='PUBLIC'`; `getPublicBookAction` strips both gates with JSDoc warning callers to use `canReadBook`; six status='PUBLISHED' refs audited and intentionally left (studio stats, feed events, profile "Published" tab — those are author-label queries, not access gates). 132/132 tests, tsc clean. Spec ✅ + Quality ✅ on all three. Pausing for Chris's per-task verification before Task 4 (hoist `ChapterList` + `CommentsPanel` + `SocialActions` from `app/[locale]/(public)/discover/_components/` up one level to `app/[locale]/(public)/_components/` so both /discover and the new /books reader can share them).
> **Active branch:** `main`
> **Last commit:** refactor(discover): use discoverable flag; strip privacy gate from getPublicBookAction
>
