...
>
> **Current focus:** **Brainstorm complete for Hive ↔ Binder integration** — full design spec at [docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md](docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md). Awaiting user review of the spec file; next step after review is plan-phase. Six decisions locked: (1) one-doc-two-windows model — single canonical row, binder + hive are two lenses; (2) whole-book auto-share when a hive is created; (3) role-based permission matrix — OWNER/EDITOR direct-edit, CONTRIBUTOR direct-add but suggest-only on edits, PROOFREADER suggest+comment, BETA_READER comment only, OWNER-only delete & role changes; (4) hive-originated artifacts appear in the binder same folder no special treatment + small avatar attribution on the row; (5) soft lock with takeover on prose artifacts (extends `hiveChapterLocks` beyond chapters), optimistic concurrency on structural ones; (6) tiered social surface — Activity tab in hive sidebar, binder stays quiet, conservative notification defaults. Schema implications flagged: `hiveOutlines` + `hiveWikiPages` retired/migrated into `binderItems`; `hiveChapterLocks` + `hiveSuggestions` extended beyond chapters; attribution columns (`createdBy`, `lastEditedBy`) added to `binderItems`. Previously shipped this session: /studio/new header eyebrow removed; /studio empty-state template chips removed; AppNav avatar dropdown with better-auth sign-out; full-page book creation wizard at /studio/new. 126/126 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** docs(hive): design spec for Hive ↔ Binder integration
>
...
>
> **Next concrete step when resuming:** Chris is finishing /studio polish first, then circling back to Hives. So the order is: (1) continue iterating on the studio surfaces (library v2, /studio/new wizard, empty state, AppNav dropdown — see recent commits for what's already touched); (2) once /studio feels done, kick off Hive ↔ Binder integration via plan-phase against the spec at [docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md](docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md). The spec is locked — do not re-litigate the six decisions there without explicit instruction; if Chris wants to revise, he'll say so. Other parallel candidates if scope shifts: Phase 9 (referral codes, growth analytics, plan-upgrade nudges), Stripe dashboard webhook configuration for live monetization, or a `/settings` index page (avatar dropdown currently links only to `/settings/billing`).
