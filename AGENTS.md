...
>
> **Current focus:** **Chapter Status — Publish-Readiness Gate spec written, awaiting Chris's spec review (`d039d72`).** Spec at [docs/superpowers/specs/2026-05-28-chapter-status-publish-gate-design.md](docs/superpowers/specs/2026-05-28-chapter-status-publish-gate-design.md). Solves "what does chapter status DO?" by making it the gate that controls reader visibility. Chapters with `status IN ('REVISED', 'FINAL')` render full prose to readers + "Updated MMM DD, YYYY" freshness label; `IDEA`/`OUTLINE`/`FIRST_DRAFT` render as "Draft — coming soon" locked teaser. Author always sees full content (gate only applies when `viewerUserId !== book.userId`). Editing doesn't auto-demote — author stays in control. Author-side surfaces: small status color dots in binder rows + explanatory paragraph and per-pill subtitles ("Visible to readers" / "Not visible to readers") in the metadata Status section. Pure helper `isChapterReaderVisible(status)` centralizes the threshold. No schema changes. Recent arc: (`b315563`) back-matter dashed outline fix via `:has()`; (`b5cf279`) TipTap placeholder for Ack/About Author; (`35af098`) `.bp-field` dashed affordance; (`dc9f08d`) note toolbar reorder; (`f240c82`) notes use full EditorToolbar; (`cc508fb`) note paper IS canvas; (`b9a4a10`) container view paper cards on lifted gray; (`ff3e711`) drag-into-folder. 157/157 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** docs(chapter-status): spec for publish-readiness gate
>
