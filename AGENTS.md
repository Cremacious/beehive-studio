...
>
> **Current focus:** **Linked Series spec approved + plan written, awaiting Chris's execution-mode pick.** Spec `0e31d9c`, plan `8087007` ([docs/superpowers/plans/2026-05-28-linked-series.md](docs/superpowers/plans/2026-05-28-linked-series.md)). 8 tasks: (1) `normalizeSeriesKey` helper + 7 tests; (2) `getSeriesNeighbors` server helper + 6 tests (mocked DB + canReadBook); (3) extend `BookSummary` projection with `seriesName`+`seriesNumber` (confirmed missing during pre-flight); (4) shared `SeriesLine` component → library / discover / profile cards; (5) library "By series" sort + clustering with subheaders ("Series Name (N books)" / "Standalone (N)"); (6) book reader page series line in hero + `<SeriesFooter>` with prev/next links; (7) wizard Step 3 + Details Structure explainer paragraphs; (8) AGENTS.md sync + final verification. Matching: author-scoped via `normalizeSeriesKey` (lowercase, strips leading "The ", collapses whitespace). Reader-side prev/next filtered through `canReadBook` so PRIVATE/FRIENDS books silently omit. No DB / schema changes. Recent arc: writing-analysis adverbs expandable (`18a277c`), Chapter-Status Publish-Gate COMPLETE. 162/162 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** docs(linked-series): implementation plan — 8 tasks, no schema changes
>
