...
>
> **Current focus:** **Hives redesign — H1 + H2 + H3 specs WRITTEN** (commits `7fc7cb7` H1, `2148ab6` H2, `8282b0d` H3; `docs/superpowers/specs/2026-05-29-h{1,2,3}-*.md`). Full Hives redesign decomposed into 5 sub-projects (H1 foundation → H2 mirror → H3 collab core → H4 motivation → H5 dashboard). H1: one-hive-per-book + standalone allowed; 4 roles (OWNER/MODERATOR/CONTRIBUTOR/BETA_READER); own visibility + new `discoverable` boolean; 3 creation paths from /studio; editor binder footer button "Create Hive"/"Go to Hive" via `getBookHive(bookId)`; /studio adds Hives section below Books; /community becomes hive-activity feed (kills `getCommunityFeedAction`); /discover/hives filters on `discoverable=true`; book delete cascades to hive; new `hive_activity` table. H2: single-source-of-truth on `binderItems` — wiki/outline/notes are ONE place, editor and hive are two views over the same rows. 14 wiki categories via new `wiki_entry` binder type + category enum + per-category templates + `wiki_folder`. Outline beats gain optional `act` field. Standalone hives back themselves with invisible `STANDALONE_HIVE_SHADOW` books rows; tightens H1's partial UNIQUE to plain UNIQUE on `hives.bookId`. New `requireBinderWritePermission`: BETA_READER read-only, CONTRIBUTOR+ writable on wiki/outline/notes; chapters/parts/FM/BM still author-only. Drops `hive_wiki_pages` + `hive_outlines`. H3: collab core. Renames `hiveComments`→`hiveAnnotations` with 5-layer enum (GRAMMAR/PLOT/TONE/CONTINUITY/GENERAL), threaded replies, dual-resolver (author OR annotator). Position drift via new TipTap mark extensions (`hiveAnnotation` + `hiveSuggestion`). `hiveSuggestions` reshaped from whole-chapter to range-targeted; `acceptSuggestionAction` does surgical range replacement. `hiveSubmissions` reshaped to carry draft TipTap content inline (DRAFT/PENDING/APPROVED/REJECTED); approval creates real `chapters` row with new `author_user_id` column → reader shows "Written by @submitter" sub-byline. `hive_discussion_posts` gains `topic` enum (GENERAL/WORLDBUILDING/FEEDBACK/OFF_TOPIC) with CHECK constraint. Drops `hive_chapter_locks`. Writes 8 event types to H1's `hive_activity` table from same-transaction inserts. **Awaiting Chris's review of all 3 specs** before deciding next path: (a) continue to H4 brainstorm (smallest remaining; locks 4 of 5 sub-projects in one session), (b) invoke writing-plans for H1+H2+H3 together, (c) invoke writing-plans for H1 only and plan H2/H3 separately. Recommendation: (a). Linked Series + Chapter Status + Drag-into-Folder + Character Redesign + Delete Book all still awaiting Chris's manual smoke (carried from prior session).
> **Active branch:** `main`
> **Last commit:** docs(spec): H3 collab core — annotations, suggestions, submissions, discussions
>
