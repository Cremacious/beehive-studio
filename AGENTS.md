...
>
> **Next concrete step when resuming:** Chris reviews the Hive-Binder integration spec at [docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md](docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md) — once approved, run plan-phase (`/gsd-plan-phase` or `writing-plans`) to produce the implementation plan. The spec is the contract; the plan-phase decides ordering, migration sequencing, and the deferred questions listed at the bottom of the spec (lock auto-release interval, suggestion granularity, attribution avatar UI, real-time freshness model). Other parallel candidates if Chris wants to defer Hive integration: Phase 9 (referral codes, growth analytics, plan-upgrade nudges), Stripe dashboard webhook configuration for live monetization, or a `/settings` index page (avatar dropdown currently links only to `/settings/billing`).
