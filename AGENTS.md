...
>
> **Current focus:** **Character profile redesign — spec approved + plan written, awaiting Chris's execution-mode pick.** Spec `29564e8`, plan `6a92719` ([docs/superpowers/plans/2026-05-28-character-profile-redesign.md](docs/superpowers/plans/2026-05-28-character-profile-redesign.md)). 2 tasks: (1) single-file rewrite of `character-profile.tsx` — convert 2-column grid to flex stack, drop per-section card chrome, add `border-t` divider rules via Tailwind `first:` variant, typography refinements (heading 17→16px, body line-height 1.65→1.7), `SectionCard.full` prop removed; (2) AGENTS.md sync. Identity header card, breadcrumb, theme tokens, MetaPill/MetaText, all logic untouched. No DB/schema/test changes. Prior: beat-delete confirmation, corkboard removal, Delete-Book epic, SP-A reader epic. 137/137 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** docs(character): implementation plan — 2 tasks, single-file rewrite
>
