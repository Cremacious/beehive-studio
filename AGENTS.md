...
>
> **Current focus:** **Hives Redesign — H3 Collaboration Core planning phase complete; execution not yet started.** H2 (18/18 tasks) shipped 2026-05-29 — see "What Has Been Built" → "Hives Redesign — H2 Mirror Model" for the full breakdown. H3 plan written 2026-06-01: 21 tasks (T1 schema migration → T21 ship) at [docs/superpowers/plans/2026-05-29-h3-collab-core.md](docs/superpowers/plans/2026-05-29-h3-collab-core.md), against the locked spec at [docs/superpowers/specs/2026-05-29-h3-collab-core-design.md](docs/superpowers/specs/2026-05-29-h3-collab-core-design.md). Plan covers annotations (TipTap mark + 5 layers + gutter + threading + resolve), edit suggestions (TipTap mark + range diff + accept/reject + threading), submit chapter (DRAFT → PENDING → APPROVED with reviewer attribution + binder slot insertion), discussions (topic-tagged threaded forum), plus migrations (rename hive_comments→hive_annotations, reshape hive_suggestions, reshape hive_submissions, add chapters.author_user_id, add hive_discussion_posts.topic + CHECK, drop hive_chapter_locks), plus 2 new TipTap marks, plus permission predicate extensions, plus activity event wiring (8 event types fired inside source-row tx). Previous session crashed twice trying to dispatch the plan-writer through the writing-plans skill; this session wrote it via a single direct general-purpose agent dispatch with a self-contained prompt. **Next concrete step:** review the plan, then kick off T1 (schema migration via `scripts/migrate-h3.ts` — idempotent migration runner matching H1/H2 style). Other candidates if H3 is deferred: (a) **H4 Engagement** (word goals + buzz board); (b) **H5 Dashboard** (hive analytics aggregation); (c) Phase 9 polish (referral codes / growth analytics / plan-upgrade nudges); (d) Stripe dashboard webhook config for live monetization; (e) /settings index page; (f) SP-B Friendships subsystem; (g) cleanup follow-ups: rename live hive_member_role.BETA_READER → READER to match docs, fix inviteLink /en/... locale hardcode in hive-members.tsx, reproject getHiveAction to join userProfiles not users.name/image, delete stale T11 stub create-hive-modal.tsx + dead create-hive-button.tsx under studio/[bookId]/_components/. Outstanding manual smokes from prior epics: H1 + H2 + Linked Series + Chapter Status + Drag-into-Folder + Character Redesign + Delete Book + SP-A reader-route checklist all still awaiting Chris.
> **Active branch:** `main`
> **Last commit:** docs(plan): H3 collab core plan — 21 tasks across annotations, suggestions, submissions, discussions
>
