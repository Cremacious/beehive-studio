...
>
> **Current focus:** **Linked Series execution — Tasks 1-6 shipped.** Plan: [docs/superpowers/plans/2026-05-28-linked-series.md](docs/superpowers/plans/2026-05-28-linked-series.md). T1 (`9849971`): series-key helper. T2 (`d99bc70`): getSeriesNeighbors. T3 (`2142f3a`): BookSummary extension. T4 (`d5bcec1`): SeriesLine + 3 cards. T5 (`f207e6f`): library "By series" sort. **T6 (`91dc22a`)**: book reader hero series line + `<SeriesFooter>` with prev/next links. `PublicBook` type + `getPublicBookAction` SELECT extended with seriesName/seriesNumber. Hero series line placed between author meta and tags row. `getSeriesNeighbors` runs in `Promise.all` with comments/progress/social fetches — no waterfall. SeriesFooter component renders null when both neighbors absent (handles non-series + lone-book series automatically). canReadBook gating inside getSeriesNeighbors so PRIVATE Book 3 in a series won't leak to non-author viewer. Spec ✅, quality ✅. **Next:** T7 = wizard Step 3 + Details Structure section explainer paragraphs (last feature task); T8 = AGENTS.md sync + final verification. 175/175 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(reader): series line in hero + previous/next footer on book reader
>
