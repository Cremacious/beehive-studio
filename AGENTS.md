...
>
> **Current focus:** **Hives Redesign — H2 Mirror Model ✅ COMPLETE** (18/18 tasks shipped — see "What Has Been Built" → "Hives Redesign — H2 Mirror Model" for the full breakdown). H2 makes the hive wiki / outline / notes BE the same binderItems rows as the editor's binder — single-source-of-truth, no sync layer. 14 wiki categories + per-category templates + tags + folder organization shipped on the editor side; the hive Wiki page renders 3 view modes (By Category / By Folder / Notes) with role-gated read/write; the hive Outline page renders the same beat sheet UI as the editor with BETA_READER read-only. Standalone hives back themselves with invisible STANDALONE_HIVE_SHADOW books rows; `scopedBooksForUser(userId)` filters them out of /studio surfaces. `requireBinderWritePermission` gates all binder writes with a 4-role × 10-type truth table. Both H1 and H2 awaiting Chris's manual smoke (carry-forward checklists in the respective "What Has Been Built" entries). **Next concrete step:** decide H3 vs other priorities. Candidates in order: (a) **H3 Collaboration Core** (chapter submission flow + edit suggestions + annotations + discussions — unlocks the chapter-author-only restriction H2 enforces; spec at 8282b0d in the H1–H5 spec stack); (b) **H4 Engagement** (word goals + buzz board); (c) **H5 Dashboard** (hive analytics aggregation); (d) Phase 9 polish (referral codes / growth analytics / plan-upgrade nudges); (e) Stripe dashboard webhook config for live monetization; (f) /settings index page; (g) SP-B Friendships subsystem; (h) cleanup follow-ups: rename live hive_member_role.BETA_READER → READER to match docs, fix inviteLink /en/... locale hardcode in hive-members.tsx, reproject getHiveAction to join userProfiles not users.name/image, delete stale T11 stub create-hive-modal.tsx + dead create-hive-button.tsx under studio/[bookId]/_components/. Outstanding from prior epics: Linked Series + Chapter Status + Drag-into-Folder + Character Redesign + Delete Book + SP-A reader-route checklist still awaiting Chris's smoke. Per-task H2 T1–T17 narrative trimmed on T18 ship — see "What Has Been Built" → "Hives Redesign — H2 Mirror Model" for the full breakdown.
> **Active branch:** `main`
> **Last commit:** feat(hive): H2 ship — mirror model live across editor + hive surfaces
>
