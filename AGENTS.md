...
>
> **Current focus:** **Drag-into-folder execution — Tasks 1-2 of 5 shipped.** Plan: [docs/superpowers/plans/2026-05-28-drag-into-folder.md](docs/superpowers/plans/2026-05-28-drag-into-folder.md). T1 (`32c9626`): pure helpers + 17 unit tests at `lib/binder/drop-rules.ts` + `lib/binder/__tests__/`. **T2 (`bf33c31`)**: `binder-tree.tsx` wired with `DropZoneState` + `dropZone` in `BinderTreeContext`; `handleDragOver` classifies zone via pointer-Y proxy (`active.rect.current.translated.top + height/2`) and gates `middle` zone on `canNest`; 500ms auto-expand timer with same-folder dedupe + cancel on every guard path; `handleDragEnd` rewritten with three branches (middle → `parentId=folderId, order=maxOrder+1`; before/after → adopt `overItem.parentId` + recompute sibling order via insertion index). `flattenVisible` retained (still consumed by `flatIds` for SortableContext). Two non-blocking concerns flagged: timer cleanup on unmount (low risk), `.find()` perf on huge binders (deferred). Spec ✅ + quality ✅. **Next:** T3 = consume `dropZone` in `binder-item.tsx`, render before/middle/after indicators (top/bottom thin brand-yellow lines + middle row ring+tint); T4 = AGENTS.md What Has Been Built entry; T5 = final verification. 154/154 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(binder): wire drop-intent state + dragOver handler
>
