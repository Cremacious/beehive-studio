...
>
> **Current focus:** **Linked Series execution — Tasks 1-2 shipped.** Plan: [docs/superpowers/plans/2026-05-28-linked-series.md](docs/superpowers/plans/2026-05-28-linked-series.md). T1 (`9849971`): `normalizeSeriesKey` helper + 7 tests. **T2 (`d99bc70`)**: `getSeriesNeighbors({ currentBook, viewerUserId })` server helper at `lib/books/get-series-neighbors.ts`. Exports `SeriesNeighbor` + `SeriesNeighbors` types. Author-scoped query (`userId = currentBook.userId AND seriesName IS NOT NULL`), JS-side normalized-key filter, per-row `canReadBook` gating, prev/next via sort + scan with strict inequality so gaps are skipped. Early returns: no seriesName → `{null, null, total:0}` skipping DB; null seriesNumber → null neighbors with correct visible total. 6 unit tests covering all branches (no seriesName, sole book, normalized-match-across-casing, gap-jumping, private-next omission, null seriesNumber). Spec ✅, quality ✅. **Next:** T3 = extend `BookSummary` projection with seriesName/seriesNumber; T4 = shared `SeriesLine` component → library/discover/profile cards; T5 = library "By series" sort + clustering; T6 = book reader hero series line + prev/next footer; T7 = wizard + Details explainer copy; T8 = AGENTS.md sync + final verification. 175/175 tests (162 baseline + 7 + 6), tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(books): getSeriesNeighbors server helper with canReadBook gating
>
