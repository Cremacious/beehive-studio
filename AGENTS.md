...
>
> **Current focus:** Brainstormed and committed the design spec for the public-facing book reader. Spec lives at [docs/superpowers/specs/2026-05-28-reader-route-privacy-discoverable-design.md](docs/superpowers/specs/2026-05-28-reader-route-privacy-discoverable-design.md). Scope was decomposed into two sub-projects: **SP-A (this spec)** = new `/[locale]/books/[bookId]` reader route + privacy enum extension (adds FRIENDS) + new `discoverable` boolean + `canReadBook()` access helper + settings UI (wizard step + Details page "Sharing" section) + editor entry points (toolbar Preview button + binder-header title link); **SP-B (separate spec, future)** = full friendships subsystem (table, request/accept/decline actions, friends list UI, FRIEND_REQUEST notifications). FRIENDS visibility is settable in SP-A but resolves to "author only" until SP-B lands. Mental model: three independent axes — `privacy` (who can open the reader), `discoverable` (whether it appears in /discover), `status` (DRAFT/PUBLISHED — author label only, no longer gates access). Migration preserves current /discover visibility: existing PUBLIC+PUBLISHED books get `discoverable=true`. Old `/discover/book/[bookId]` URLs 308-redirect to the new route; reader components (`ChapterList`, `CommentsPanel`, `SocialActions`) hoist one level up to be shared between both paths. Spec approved section-by-section. Awaiting Chris's review of the written spec before /writing-plans.
> **Active branch:** `main`
> **Last commit:** docs(reader): spec for /books/[bookId] reader route + privacy/discoverable
>
...
>
> **Next concrete step when resuming:** Chris is reviewing the SP-A reader-route spec ([docs/superpowers/specs/2026-05-28-reader-route-privacy-discoverable-design.md](docs/superpowers/specs/2026-05-28-reader-route-privacy-discoverable-design.md)). Once approved, invoke `writing-plans` to draft the implementation plan, then execute. SP-A is self-contained — DOES NOT block on SP-B friendships (FRIENDS visibility resolves to author-only until SP-B ships, by design). Do not re-litigate the eight design sections without explicit instruction; if Chris wants to revise, he'll say so. After SP-A ships, the next decisions in order are: (a) SP-B Friendships subsystem (separate brainstorm — symmetric request/accept like the old beehive-books-online had); (b) Hive ↔ Binder integration via the locked spec at [docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md](docs/superpowers/specs/2026-05-27-hive-binder-integration-design.md); (c) Phase 9 polish (referral codes, growth analytics, plan-upgrade nudges); (d) Stripe dashboard webhook configuration for live monetization; (e) a `/settings` index page.
