...
>
> **Current focus:** **Hives redesign — FULL SPEC STACK COMPLETE (H1–H5)** (commits: `7fc7cb7` H1, `2148ab6` H2, `8282b0d` H3, `b742908` milestone-removal amendment, `f1767a9` H4, `d11db57` H5; all in `docs/superpowers/specs/2026-05-29-h{1,2,3,4,5}-*.md`). All 5 sub-projects of the Hives redesign now specified end-to-end. H1 foundation: one-hive-per-book + standalone allowed; 4 roles (OWNER/MOD/CONTRIBUTOR/BETA_READER); own visibility + `discoverable`; 3 creation paths from /studio; editor binder button "Create Hive"/"Go to Hive"; /studio adds Hives section; /community becomes hive-activity feed; /discover/hives filters on `discoverable`; book delete cascades; new `hive_activity` table. H2 mirror: single-source-of-truth on `binderItems` — wiki/outline/notes are ONE place. 14 wiki categories via `wiki_entry` + category enum + templates + `wiki_folder`. Outline gains `act` field. Standalone hives via invisible `STANDALONE_HIVE_SHADOW` books rows; tightens H1 to plain UNIQUE on `hives.bookId`. New `requireBinderWritePermission`. Drops `hive_wiki_pages` + `hive_outlines`. H3 collab core: `hiveAnnotations` with 5-layer enum + threaded replies + dual-resolver; TipTap mark extensions for drift. `hiveSuggestions` range-targeted with surgical accept. `hiveSubmissions` carries draft TipTap content inline (DRAFT→PENDING→APPROVED/REJECTED); approval creates real chapters with `author_user_id` → reader shows "Written by @submitter". `hive_discussion_posts` gains `topic` enum + CHECK constraint. Drops `hive_chapter_locks`. Writes 8 event types to `hive_activity`. **Mid-brainstorm pivot:** Milestones feature DROPPED entirely; H1+H2+H3 specs amended (`b742908`). H4 motivation: `hive_word_goals` (DAILY/WEEKLY/MONTHLY/TOTAL, one active per type, auto-archives on new); `hive_word_logs` via 60s-throttled hook on `saveChapterAction` (deltas; negatives allowed). `/word-goals` page + sidebar badge. `hive_buzz_posts` (TEXT/LINK only; no images, no comments) + `hive_buzz_likes` with denormalized `like_count`. H5 dashboard: activity-first landing surface at `/hive/[hiveId]`. Hero + activity feed + role-conditional "Needs your attention" queue + active goal + linked book stats + recent buzz. One composite `getHiveDashboardAction` read (~7 sub-queries + 1 recursive CTE). New `hive_member_last_seen` table tracks watermark for "replies since last visit" semantics. Standalone hives render the same surface with linked-book panel hidden. **Awaiting Chris's review of all 5 specs** before deciding ship cadence: (a) writing-plans on H1 only (recommended — per-task verification, smallest verifiable surface, leaves H2's tightening as a clean follow-up), (b) writing-plans on H1+H2 together (tighter migration but bigger plan), (c) writing-plans on all 5 sequentially (most up-front planning). Linked Series + Chapter Status + Drag-into-Folder + Character Redesign + Delete Book all still awaiting Chris's manual smoke (carried from prior session).
> **Active branch:** `main`
> **Last commit:** docs(spec): H5 dashboard — closes the Hives redesign spec cycle
>
