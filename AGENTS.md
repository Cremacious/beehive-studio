...
>
> **Current focus:** **Drag-into-folder — spec written, awaiting Chris's review before plan-phase.** Spec at [docs/superpowers/specs/2026-05-28-drag-into-folder-design.md](docs/superpowers/specs/2026-05-28-drag-into-folder-design.md) (`4c3f3e3`). Extends binder drag-and-drop with nesting. Type-strict accept rules: `part` accepts `chapter`+`part`; `research_folder` accepts `research_note`+`research_folder`; others top-level only. Three drop zones per folder row: top edge (~6px) reorders before, middle body nests inside, bottom edge (~6px) reorders after. Visual indicators: thin brand-yellow lines for before/after, brand-yellow ring + `bg-brand/10` tint on row for nest. Auto-expand collapsed folders after ~500ms middle-body hover. Pure validation helpers (`getAcceptedChildTypes`, `canNest`, `classifyDropZone`) in new `lib/binder/drop-rules.ts` for unit testability. Cycle guard via parentId walk. No DB / schema changes — `reorderBinderItemsAction` already accepts arbitrary `{id, order, parentId}` tuples (pre-flight verify in plan). Prior: drag-anywhere (`ab639f1`), character redesign, beat-delete confirmation, corkboard removal, Delete-Book epic, SP-A reader epic. 137/137 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** docs(binder): spec for drag-into-folder with three-zone drop intent
>
