...
>
> **Current focus:** **Chapter-Status Publish-Gate spec approved + plan written, awaiting Chris's execution-mode pick.** Spec `d039d72`, plan `f149a5b` ([docs/superpowers/plans/2026-05-28-chapter-status-publish-gate.md](docs/superpowers/plans/2026-05-28-chapter-status-publish-gate.md)). 8 tasks: (1) `isChapterReaderVisible` helper + 5 unit tests; (2) thread `status`+`updatedAt` through the book reader page (chapter projection + `isAuthor` flag); (3) `ChapterList` gates by status, renders "Draft — coming soon" badge for locked / "Updated MMM DD, YYYY" label for visible; (4) chapter reader page locked placeholder for direct-URL access; (5) binder color dots for chapter rows (may extend `BinderItemRow` projection to expose status); (6) metadata panel explainer paragraph + per-pill "Visible/Not visible to readers" subtitles; (7) AGENTS.md sync; (8) final verification. Tasks 2+3 tightly coupled — recommended single combined commit. No schema/server-action shape changes. Recent arc: (`b315563`) back-matter dashed outline `:has` fix; (`b5cf279`) TipTap placeholder; (`35af098`) `.bp-field` dashed affordance; (`dc9f08d`) note toolbar reorder; (`f240c82`) notes use full EditorToolbar; (`cc508fb`) note paper IS canvas; (`b9a4a10`) container view paper cards; (`ff3e711`) drag-into-folder. 157/157 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** docs(chapter-status): implementation plan — 8 tasks, no schema changes
>
