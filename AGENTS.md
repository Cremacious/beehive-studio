...
>
> **Current focus:** **Character profile redesign COMPLETE (`8ec4eca`)** — single-file rewrite (+11/-39). 2-column section grid replaced with single-column flex stack + thin `var(--sheet-rule)` divider lines via Tailwind `first:` variant. Per-section card chrome (background/shadow/radius) dropped from both `SectionCard` and the Relationships outer wrapper. `<div>`→`<section>` semantic upgrade. Typography refinements (heading 17→16, body line-height 1.65→1.7). `SectionCard.full` prop removed. Identity header card, breadcrumb, theme tokens, MetaPill/MetaText, all logic untouched. Eliminates the Appearance↔Personality row-stretch bug entirely. Prior: beat-delete confirmation, corkboard removal, Delete-Book epic, SP-A reader epic. 137/137 tests, tsc clean. **Awaiting Chris's manual smoke** (open existing character, type long Personality, verify only Personality grows; verify dividers visible in both themes; verify identity header unchanged).
> **Active branch:** `main`
> **Last commit:** feat(character): single-column specimen-sheet redesign
>
