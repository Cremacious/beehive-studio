...
>
> **Current focus:** **Drag-into-folder — spec approved + plan written, awaiting Chris's execution-mode pick.** Spec `4c3f3e3`, plan `f9f2f55` ([docs/superpowers/plans/2026-05-28-drag-into-folder.md](docs/superpowers/plans/2026-05-28-drag-into-folder.md)). 5 tasks: (1) pure helpers in new `lib/binder/drop-rules.ts` (`getAcceptedChildTypes`, `canNest` with cycle guard, `classifyDropZone`) + ~18 unit tests; (2) wire drop-intent state + dragOver handler + auto-expand timer + intent-driven handleDragEnd in `binder-tree.tsx`; (3) render before/middle/after drop indicators in `binder-item.tsx` via extended `BinderTreeContext`; (4) AGENTS.md sync; (5) final verification. Pre-flight confirmed `reorderBinderItemsAction` accepts arbitrary `{id, order, parentId}` tuples — no server changes. Type-strict accept rules: part accepts chapter+part; research_folder accepts research_note+research_folder; others top-level only. Three drop zones per folder row + auto-expand collapsed folders on ~500ms middle-body hover. Prior: drag-anywhere, character redesign, beat-delete confirmation, corkboard removal, Delete-Book epic, SP-A reader epic. 137/137 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** docs(binder): implementation plan for drag-into-folder — 5 tasks
>
