...
>
> **Current focus:** **Linked Series spec written, awaiting Chris's spec review (`0e31d9c`).** Spec at [docs/superpowers/specs/2026-05-28-linked-series-design.md](docs/superpowers/specs/2026-05-28-linked-series-design.md). Series choice currently writes `seriesName` + `seriesNumber` but those fields are never displayed — feature is write-only. Spec gives series load-bearing jobs: (1) "Book N of <Series>" line on reader page, library card, discover card, /u/[username] profile; (2) library "By series" sort clusters books with subheaders; (3) book reader page exposes previous/next navigation between books in same series, gated through `canReadBook`; (4) wizard Step 3 + Details Structure section get explanatory copy. Matching via new pure helper `normalizeSeriesKey` (case-insensitive, drops leading "The ", collapses whitespace) — author-scoped, so two unrelated "Chronicles" by different authors don't collide. New server helper `getSeriesNeighbors` returns prev/next/total filtered by viewer access. No DB/schema changes; `BookSummary` projection may need to expose seriesName/seriesNumber if missing. Recent arc: writing-analysis adverbs expandable (`18a277c`), Chapter-Status Publish-Gate COMPLETE (6 commits). 162/162 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** docs(linked-series): spec for surfacing + linking series metadata
>
