...
>
> **Current focus:** **Linked Series execution — Tasks 1-3 shipped.** Plan: [docs/superpowers/plans/2026-05-28-linked-series.md](docs/superpowers/plans/2026-05-28-linked-series.md). T1 (`9849971`): `normalizeSeriesKey` helper + 7 tests. T2 (`d99bc70`): `getSeriesNeighbors` server helper + 6 tests. **T3 (`2142f3a`)**: `BookSummary` type + `getUserBooksAction` projection extended with `seriesName: string | null` + `seriesNumber: number | null`. Pre-flight had flagged these were missing; confirmed. Single literal construction site in book.actions.ts (only that map needed updating — other consumers are type-only). Spec ✅, quality ✅. **Next:** T4 = shared `SeriesLine` component (library card + discover card + /u/[username] profile tile); T5 = library "By series" sort + clustering with subheaders; T6 = book reader hero series line + `<SeriesFooter>` prev/next; T7 = wizard + Details explainer copy; T8 = AGENTS.md sync + final verification. 175/175 tests, tsc clean.
> **Active branch:** `main`
> **Last commit:** feat(books): BookSummary projection includes seriesName + seriesNumber
>
