...
function ctaHref(
  ...
  switch (variant) {
    ...
    case 'connect-friends':
      return `/${locale}/community/friends`
    case 'prompt-template': {
      if (!promptTemplate) return `/${locale}/community/sparks/new`
      const sp = new URLSearchParams()
      sp.set('prompt', promptTemplate.prompt)
      sp.set('wordLimit', String(promptTemplate.wordLimit))
      return `/${locale}/community/sparks/new?${sp.toString()}`
    }
    case 'enter-a-spark':
      return `/${locale}/discover?tab=sparks`
    case 'create-first':
      return `/${locale}/community/sparks/new`
  }
}
