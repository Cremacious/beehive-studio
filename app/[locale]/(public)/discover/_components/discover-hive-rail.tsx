...
import type { HiveCard, RailResult } from '@/lib/actions/discover-hives.actions'
...
export function DiscoverHiveRail({
  title,
  subPageHref,
  result,
  locale,
  ...
  return (
    <section style={panelChrome} className="mb-6">
      ...
      {result.strictCount < 4 ? (
        ...
        >
          Filling in with recently active Hives while {title} warms up.
        </p>
      ) : null}
      <div className="overflow-x-auto px-5 pb-5">
        <ul className="flex gap-3 snap-x snap-mandatory">
          {result.books.map((hive) => (
            <li key={hive.id} className="snap-start shrink-0">
              <RailHiveCard hive={hive} locale={locale} />
            </li>
          ))}
        </ul>
      </div>
