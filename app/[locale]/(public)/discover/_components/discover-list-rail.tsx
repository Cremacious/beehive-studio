...
export function DiscoverListRail({
  title,
  subPageHref,
  result,
  locale,
  ...
  return (
    <section style={panelChrome} className="mb-6">
      ...
      {result.strictCount < 4 ? (
        ...
        >
          Filling in with active Lists while {title} warms up.
        </p>
      ) : null}
      <div className="overflow-x-auto px-5 pb-5">
        <ul className="flex gap-3 snap-x snap-mandatory">
          {result.books.map((list) => (
            <li key={list.id} className="snap-start shrink-0">
              <RailListCard list={list} locale={locale} />
            </li>
          ))}
        </ul>
      </div>
