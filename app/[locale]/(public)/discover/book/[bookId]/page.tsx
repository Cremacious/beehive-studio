...
export default async function BookDetailPage({ params }: Props) {
  ...
  // Normalize potentially-null wordCount from DB
  const normalizedChapterRows = chapterRows.map(ch => ({
    ...ch,
  ...
  return (
    <div className="min-h-screen bg-[#141414]">
      ...
      <div className="bg-[#1a1a1a] border-b border-[#2a2a2a] px-6 py-3 flex items-center gap-3">
        <Link href={`/${locale}/discover`} className="text-[#888] text-[13px] hover:text-white transition-colors">
          ← Discover
        </Link>
      </div>
        ...
        <div className="flex flex-col justify-between">
          ...
          <div className="mt-4 flex items-center gap-2.5 flex-wrap">
            {normalizedChapterRows[0] && (
              <Link
                href={`/${locale}/discover/book/${bookId}/read/${lastReadChapter?.chapterId ?? normalizedChapterRows[0].chapterId}`}
      ...
      {progress?.lastChapterId && (
        <div className="px-6 py-3 bg-[#181818] border-b border-[#2a2a2a] flex items-center gap-3">
          ...
          {lastReadChapter && (
            <Link
              href={`/${locale}/discover/book/${bookId}/read/${lastReadChapter.chapterId}`}
