...
import { getRisingStarsBooksAction } from '@/lib/actions/discover.actions'
...
export default async function RisingPage({ params, searchParams }: Props) {
  const { locale } = await params
  const sp = await searchParams
  const genre = sp.genre && isValidGenre(sp.genre) ? sp.genre : undefined
  const result = await getRisingStarsBooksAction({ genre, cursor: sp.cursor })

  if (!result.success) {
    return (
      <main className="max-w-5xl mx-auto px-4 py-6">
        <p className="text-[13px] text-[var(--canvas-dark-ink-muted)]">
          Failed to load Rising Stars. Try again later.
        </p>
      </main>
    )
  }
